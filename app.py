...
app = FastAPI(title="AION2 Server v7 AllServer")
...
# =========================================================
# Character search - ALL Korean servers
# =========================================================
...
async def search_characters_all_servers(nickname: str):
    ...
    for row in results:
        if not isinstance(row, dict):
            continue
        row_name = str(row.get("name") or row.get("characterName") or "").strip()
        if row_name.casefold() == target:
            exact.append(row)

    # Exact spelling/case first, then stable server order.
    exact.sort(
        key=lambda x: (
            0 if str(x.get("name") or x.get("characterName") or "").strip() == nickname else 1,
            int(x.get("serverId") or 999999),
        )
    )
    return exact

def candidate_server_name(row):
    return str(
        row.get("serverName")
        or row.get("server")
...
async def get_profile(server_id: int, character_id: str):
    # IMPORTANT:
    # profile does NOT use fast=1. The fast profile can omit equipment/itemDetails,
    # which causes magic stones to disappear.
...
def _collect_stone_lists(node, out, depth=0):
    """Find magicStoneStat robustly even if NotMeter moves itemDetails."""
    if depth > 8:
        return
...
def aggregate_magic_stones(profile_json):
    ...
    # Normal profile schema first.
    item_details = profile_json.get("itemDetails")
    if isinstance(item_details, dict):
        for item in item_details.values():
            if isinstance(item, dict) and isinstance(item.get("magicStoneStat"), list):
                stone_lists.append(item["magicStoneStat"])
    elif isinstance(item_details, list):
        for item in item_details:
            if isinstance(item, dict) and isinstance(item.get("magicStoneStat"), list):
                stone_lists.append(item["magicStoneStat"])

    # Fallback for schema movement/wrapping.
    if not stone_lists:
        _collect_stone_lists(profile_json, stone_lists)

    # Avoid accidental duplicate list objects while recursively walking.
    seen_lists = set()
    unique_lists = []
    for stones in stone_lists:
        ...
        unique_lists.append(stones)

    for stones in unique_lists:
        for stone in stones:
...
def profile_info(profile_json, requested_name="", fallback_server=""):
    profile = profile_json.get("info", {}).get("profile", {}) or {}
    return {
        "name": profile.get("characterName") or requested_name,
        "job": profile.get("className") or "확인 실패",
        "combatPower": int(profile.get("combatPower") or 0),
        "serverId": int(profile.get("serverId") or 0),
        "server": profile.get("serverName") or fallback_server or "확인 실패",
        "level": int(profile.get("characterLevel") or 0),
        "race": profile.get("raceName") or "",
        "title": profile.get("titleName") or "",
        "profileImage": profile.get("profileImage") or "",
    }
...
def format_character(profile_json, requested_name, fallback_server=""):
    info = profile_info(profile_json, requested_name, fallback_server)
    cp_short = round(info["combatPower"] / 1000) if info["combatPower"] else 0
    stones = aggregate_magic_stones(profile_json)

    lines = [
        "╭━━ ⚔️ AION2 캐릭터 ━━",
        f"┃ {info['name']}  ·  {info['server']}",
        f"┃ {info['job']}  ·  전투력 {cp_short}",
    ]
    if info["level"]:
        lines.append(f"┃ Lv.{info['level']}")
    lines += [
        "╰━━━━━━━━━━━━━━",
        "",
        "💎 장착 마석 총합",
...
async def resolve_character(nickname: str, server_name: str | None = None):
    ...
    if server_name:
        target_server = server_name.casefold()
        filtered = [
            row for row in candidates
            if candidate_server_name(row).casefold() == target_server
        ]

        # Search rows sometimes omit serverName, so inspect profiles if needed.
        if not filtered and candidates:
            inspected = await asyncio.gather(
                *[fetch_candidate_profile(row) for row in candidates[:20]]
            )
            filtered = [
                x["row"] for x in inspected
                if x and str(x["info"].get("server") or "").casefold() == target_server
            ]

        candidates = filtered

    if not candidates:
        return {"type": "none", "nickname": nickname, "server": server_name}

    # Unique candidate -> full detail.
    if len(candidates) == 1:
        row = candidates[0]
        detail = await fetch_candidate_profile(row)
        if not detail:
            return {"type": "none", "nickname": nickname, "server": server_name}
        return {"type": "detail", **detail}

    # Duplicate nickname across servers: return compact ALL-server list.
    details = await asyncio.gather(
        *[fetch_candidate_profile(row) for row in candidates[:12]]
    )
    ...
    details.sort(key=lambda x: x["info"]["combatPower"], reverse=True)
    return {"type": "multiple", "nickname": nickname, "items": details}

def format_character_multiple(nickname, items):
    lines = [
        f"🔎 {nickname} · 전 서버 검색",
        f"동일 닉네임 {len(items)}명",
        "",
    ]
    ...
    for idx, item in enumerate(items[:10], 1):
        ...
        cp_short = round(info["combatPower"] / 1000) if info["combatPower"] else 0
        lines.append(
            f"{idx}. {info['server']} · {info['job']} · 전투력 {cp_short}"
        )
...
async def character_lookup(nickname_query: str):
    ...
    cache_key = f"char-all:{nickname}:{server_name or '*'}"
...
async def character_card_data(nickname: str, server_name: str):
    cache_key = f"card:{nickname}:{server_name}"
    cached = cache_get(cache_key)
    if cached:
        return cached
    ...
    data = {
        "info": resolved["info"],
        "stones": aggregate_magic_stones(resolved["profile"]),
    }
...
async def root():
    return {
        "ok": True,
        "service": "AION2 Server v7 AllServer",
...
async def openchat(msg: str = ""):
    ...
    # Character
    try:
        result = await asyncio.wait_for(character_lookup(body), timeout=8.5)
    except asyncio.TimeoutError:
