...
.equip-detail-compare{display:grid;grid-template-columns:1fr 1fr;gap:14px}.equip-detail-owner{border:1px solid var(--line);border-radius:14px;padding:12px;background:#0b1524}.equip-detail-owner.a{border-color:#365f97}.equip-detail-owner.b{border-color:#674886}.equip-detail-head{font-size:16px;font-weight:900;margin-bottom:10px}.equip-detail-list{display:grid;gap:10px}.equip-detail-card{display:grid;grid-template-columns:minmax(0,1.2fr) minmax(190px,.8fr);gap:14px;border:1px solid #263851;border-radius:12px;background:#0e1a2b;padding:10px}.equip-detail-left{display:grid;grid-template-columns:52px minmax(0,1fr);gap:10px}.equip-big-icon{width:52px;height:52px;border-radius:8px;border:1px solid #31435f;background:#142033;overflow:hidden}.equip-big-icon img{width:100%;height:100%;object-fit:cover}.equip-item-title{font-size:13px;font-weight:900;color:#ff7a2f}.equip-item-slot{font-size:10px;color:#7fddff;margin-bottom:3px}.equip-mainstats{display:grid;grid-template-columns:1fr 1fr;gap:3px 12px;margin-top:8px}.equip-mainstat{font-size:11px;color:#dbe7f7}.equip-right{border-left:1px solid #2a5267;padding-left:10px;display:grid;gap:4px;align-content:start}.equip-substat{display:flex;justify-content:space-between;gap:8px;border:1px solid #20475b;border-radius:6px;padding:4px 7px;font-size:11px;background:#09202b}.equip-substat b{color:#fff}.equip-empty{color:var(--muted);padding:10px}.stone-diff-wrap th,.stone-diff-wrap td{font-size:12px}
...
function renderEquipmentOwner(character, headId, listId){
  ...
  $(listId).innerHTML=rows.map(r=>{
    ...
    return `<div class="equip-detail-card"><div class="equip-detail-left"><div class="equip-big-icon">${icon}</div><div><div class="equip-item-slot">${E(r?.slot||'장비')}</div><div class="equip-item-title">${E(enh+(r?.name||'-'))}${E(exceed)}</div><div class="equip-mainstats">${main.map(s=>`<div class="equip-mainstat">${E(statText(s))}</div>`).join('')}</div></div></div><div class="equip-right">${subs.length?subs.map(s=>`<div class="equip-substat"><span>${E(s?.name||s?.id||'-')}</span><b>${E(String(s?.value??'-'))}</b></div>`).join(''):'<div class="equip-empty">추가 옵션 없음</div>'}</div></div>`;
  }).join('');
}
