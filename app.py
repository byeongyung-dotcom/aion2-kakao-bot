...
async def openchat_alerts(room: str = ""):
    async with _openchat_alert_lock:
        ...
        # ---- Boss/content alerts: 30 min + 10 min before, once PER ROOM ----
        now = datetime.now(KST)
        ...
        for item_type, name, target in targets:
            ...
            # Keep both lead-time dedupe keys around until well after the event.
            if minutes > -180:
                keep.add(event_id + "|30")
                keep.add(event_id + "|10")

            # 1-minute phone polling + Render wake-up delay tolerance.
            # Every boss/content schedule gets exactly two alerts: 30m and 10m.
            for lead, low, high in ((30, 27, 33), (10, 7, 13)):
                key = event_id + f"|{lead}"
                if low <= minutes <= high and key not in sent:
                    items.append({
                        "type": item_type,
