...
app = FastAPI(title="AION2 Server v56 EquipFoldSkillFilter")
    ...
    def scan_container(node, priority=100, prefix=""):
        if isinstance(node, dict):
            ...
            if row_name:
                ...
                add(row_name, raw, node.get("key") or node.get("id") or prefix, priority)
...
<div class="wrap">
  ...
  <div class="panel section">
    ...
    <table>
      ...
      <tbody id="attackStats">
        ...
        <tr data-stat="공격력"><td>공격력</td><td>18,420</td><td>19,180</td><td class="bad">-760</td></tr>
        <tr data-stat="추가 공격력"><td>추가 공격력</td><td>1,240</td><td>1,380</td><td class="bad">-140</td></tr>
        <tr data-stat="PVE 공격력"><td>PVE 공격력</td><td>2,040</td><td>2,260</td><td class="bad">-220</td></tr>
        <tr data-stat="보스 공격력"><td>보스 공격력</td><td>860</td><td>940</td><td class="bad">-80</td></tr>

        <tr class="group"><td colspan="4">피해 증폭</td></tr>
        <tr data-stat="피해 증폭"><td>일반 피해증폭</td><td>13.2%</td><td>14.8%</td><td class="bad">-1.6%p</td></tr>
        <tr data-stat="무기 피해 증폭"><td>무기 피해증폭(무피)</td><td>11.4%</td><td>14.1%</td><td class="bad">-2.7%p</td></tr>
        <tr data-stat="PVE 피해 증폭"><td>PVE 피해증폭</td><td>9.8%</td><td>10.9%</td><td class="bad">-1.1%p</td></tr>
        <tr data-stat="보스 피해 증폭"><td>보스 피해증폭</td><td>4.2%</td><td>5.0%</td><td class="bad">-0.8%p</td></tr>
        <tr data-stat="전방 피해 증폭"><td>전방 피해증폭(전피)</td><td>0%</td><td>0%</td><td>0</td></tr>
        <tr data-stat="후방 피해 증폭"><td>후방 피해증폭(후피)</td><td>17.2%</td><td>18.8%</td><td class="bad">-1.6%p</td></tr>
        <tr data-stat="치명타 피해 증폭"><td>치명타 피해증폭</td><td>18.8%</td><td>20.1%</td><td class="bad">-1.3%p</td></tr>

        <tr class="group"><td colspan="4">조건·확률</td></tr>
        <tr data-stat="치명타"><td>치명타</td><td>4,120</td><td>4,240</td><td class="bad">-120</td></tr>
        <tr data-stat="강타"><td>강타</td><td>21.5</td><td>22.8</td><td class="bad">-1.3</td></tr>
        <tr data-stat="명중"><td>명중</td><td>5,180</td><td>5,310</td><td class="bad">-130</td></tr>
        <tr data-stat="관통"><td>관통</td><td>720</td><td>760</td><td class="bad">-40</td></tr>
        <tr data-stat="공격 속도"><td>공격 속도</td><td>18.0%</td><td>18.0%</td><td>0</td></tr>
        <tr data-stat="시전 속도"><td>시전 속도</td><td>22.0%</td><td>24.0%</td><td class="bad">-2.0%p</td></tr>
      </tbody>
  ...
  <div class="panel section">
    ...
    <div class="equip-toggle-row">
      <button class="mini-btn" id="toggleEquipA">A 장비 펼치기</button>
      <button class="mini-btn" id="toggleEquipB">B 장비 펼치기</button>
    </div>
    <div class="split">
      <div class="owner a">
        <div class="ownerhead" id="eqHeadA">A · 윤이 · 마도성</div>
        <div class="cards equip-collapsed" id="equipA">
      ...
      <div class="owner b">
        ...
        <div class="cards equip-collapsed" id="equipB">
...
function renderAttack(a,b){
  ...
  document.querySelectorAll("#attackStats tr[data-stat]").forEach(tr=>{
    const key=tr.dataset.stat, av=valueOf(A[key]), bv=valueOf(B[key]);
    const unit=(A[key]?.unit==="percent"||B[key]?.unit==="percent")?"%":"";
    const tds=tr.querySelectorAll("td");
    tds[1].textContent=av===null?"데이터 없음":`${F(av)}${unit}`;
    tds[2].textContent=bv===null?"데이터 없음":`${F(bv)}${unit}`;
    tds[3].className="";
    if(av===null||bv===null){tds[3].textContent="—";return}
    const d=av-bv;
    if(d<0)tds[3].classList.add("bad"); else if(d>0)tds[3].classList.add("good");
    tds[3].textContent=`${d>0?"+":""}${F(d)}${unit?"%p":""}`;
  });
}
...
function detailText(x){
  ...
  (x?.options||[]).forEach(o=>lines.push(`${o.name||""} ${o.value??""}`.trim()));
  (x?.magicStones||[]).forEach(s=>lines.push(`💎 ${s.name||"마석"} ${s.value??""}`.trim()));
  return lines.join(" · ")||"추가 상세정보 없음";
}
function cardHTML(x,type){
  const data=encodeURIComponent(JSON.stringify(x||{}));
  const icon=x?.icon?`<img class="icon" src="${E(x.icon)}" style="object-fit:cover">`:`<div class="icon">${type==="장비"?"⚔️":type==="아르카나"?"✦":"◆"}</div>`;
  const meta=[
    x?.enhance!=null?`+${x.enhance}`:"",
    x?.grade||"",
    x?.level!=null?`Lv.${x.level}`:"",
    x?.slot||""
  ].filter(Boolean).join(" · ");
  const opts=(x?.options||[]).slice(0,2).map(o=>`${o.name||""} ${o.value??""}`.trim()).join(" · ");
  const stones=(x?.magicStones||[]).slice(0,2).map(s=>`${s.name||"마석"} ${s.value??""}`.trim()).join(" · ");
  const tiny=[meta,opts,stones].filter(Boolean).join(" · ")||type;
  return `<div class="card" data-detail="${data}"><div class="rowcard">${icon}<div><div class="ctitle">${E(x?.name||"데이터 없음")}</div><div class="tiny">${E(tiny)}</div></div></div></div>`;
}
function emptyCards(type,count){
  return Array.from({length:count},(_,i)=>`<div class="card"><div class="rowcard"><div class="icon">${type==="장비"?"⚔️":"✦"}</div><div><div class="ctitle">데이터 없음</div><div class="tiny">${type} 상세 조회 대기</div></div></div></div>`).join("");
}
function wireCards(){
  document.querySelectorAll("[data-detail]").forEach(el=>{
    el.onclick=()=>{
      try{
        const x=JSON.parse(decodeURIComponent(el.dataset.detail));
        show(x.name||"-",[x.slot,x.grade,x.level!=null?`Lv.${x.level}`:"",x.enhance!=null?`+${x.enhance}`:""].filter(Boolean).join(" · "),detailText(x));
      }catch(e){console.error(e)}
    };
  });
}

function renderEquipmentArcana(d){
  const a=d.a,b=d.b;
  $("eqHeadA").textContent=`A · ${a.info?.name||"-"} · ${a.info?.job||"-"}`;
  $("eqHeadB").textContent=`B · ${b.info?.name||"-"} · ${b.info?.job||"-"}`;
  $("equipA").innerHTML=(a.equipment||[]).map(x=>cardHTML(x,"장비")).join("")||emptyCards("장비",8);
  $("equipB").innerHTML=(b.equipment||[]).map(x=>cardHTML(x,"장비")).join("")||emptyCards("장비",8);

  $("arcHeadA").textContent=`A · ${a.info?.name||"-"}`;
  $("arcHeadB").textContent=`B · ${b.info?.name||"-"}`;
  $("arcA").innerHTML=(a.arcana||[]).map(x=>cardHTML(x,"아르카나")).join("")||emptyCards("아르카나",3);
  $("arcB").innerHTML=(b.arcana||[]).map(x=>cardHTML(x,"아르카나")).join("")||emptyCards("아르카나",3);
}
...
function renderLevelTable(a,b,type){
  ...
  const ra=filterLevelRows(passive?(a.passives||[]):(a.skills||[])), rb=filterLevelRows(passive?(b.passives||[]):(b.skills||[]));
  ...
  $(passive?"passiveAName":"activeAName").textContent=a.info?.name||"A";
  $(passive?"passiveBName":"activeBName").textContent=b.info?.name||"B";
  title.textContent=same?`${a.info?.job||""} ${passive?"패시브":"핵심 액티브 스킬"} 1:1 비교`:`직업별 ${passive?"패시브":"액티브"} 투자 비교`;

  if(same){
    const names=[...new Set([...Object.keys(ma),...Object.keys(mb)])].sort();
    body.innerHTML=names.map(n=>{
      const av=Number(ma[n]?.level),bv=Number(mb[n]?.level),validA=Number.isFinite(av),validB=Number.isFinite(bv);
      const d=validA&&validB?av-bv:null, cls=d<0?"bad":d>0?"good":"";
      return `<tr><td>${E(n)}</td><td>${validA?av:"—"}</td><td>${validB?bv:"—"}</td><td class="${cls}">${d===null?"—":`${d>0?"+":""}${d}`}</td></tr>`;
    }).join("")||`<tr><td colspan="4">실제 ${passive?"패시브":"액티브"} 데이터 없음</td></tr>`;
  }else{
    const n=Math.max(ra.length,rb.length);
    body.innerHTML=Array.from({length:n},(_,i)=>`<tr><td>${E(ra[i]?.name||rb[i]?.name||"-")}</td><td>${ra[i]?.level??"—"}</td><td>${rb[i]?.level??"—"}</td><td>—</td></tr>`).join("")||`<tr><td colspan="4">데이터 없음</td></tr>`;
  }
  ...
  if(k.length>=4){
    ...
    k[3].className="value";
    k[3].textContent=same?"동일명 1:1":"직업별 투자";
  }
}

function renderBottomCards(d){
  $("skillHeadA").textContent=`A · ${d.a.info?.name||"-"} · ${d.a.info?.job||"-"}`;
  $("skillHeadB").textContent=`B · ${d.b.info?.name||"-"} · ${d.b.info?.job||"-"}`;
  const aRows=[...filterLevelRows(d.a.skills||[]),...filterLevelRows(d.a.passives||[])];
  const bRows=[...filterLevelRows(d.b.skills||[]),...filterLevelRows(d.b.passives||[])];
  $("skillsA").innerHTML=aRows.map(x=>cardHTML(x,x.category||"스킬")).join("")||emptyCards("스킬",2);
  $("skillsB").innerHTML=bRows.map(x=>cardHTML(x,x.category||"스킬")).join("")||emptyCards("스킬",2);
}
...
function render(d){
  ...
  document.querySelectorAll("table").forEach(tbl=>{
    ...
    if(th.length>=4){th[1].textContent=d.a.info?.name||"A";th[2].textContent=d.b.info?.name||"B";}
  });

  renderGap(d);
  renderAttack(d.a,d.b);
  renderPriority(d.proAnalysis||{});
  renderEquipmentArcana(d);
...
async function compare(){
  ...
  try{
    setEquipOpen("A",false);
    setEquipOpen("B",false);
    ...
    $("loadStatus").textContent=`${d.a.info?.name||A.name} ↔ ${d.b.info?.name||B.name} 비교 완료 · 장비 ${ha.equipmentCount||0}/${hb.equipmentCount||0} · 스탯 ${ha.statCount||0}/${hb.statCount||0} · 아르카나 ${ha.arcanaCount||0}/${hb.arcanaCount||0}`;
  }catch(e){
    console.error(e); $("loadStatus").textContent="비교 서버 연결 오류";
  }finally{
    $("compareBtn").disabled=false;
  }
}

function setEquipOpen(side,open){
  const cards=$(side==="A"?"equipA":"equipB");
  const btn=$(side==="A"?"toggleEquipA":"toggleEquipB");
  if(!cards||!btn)return;
  cards.classList.toggle("equip-collapsed",!open);
  btn.textContent=`${side} 장비 ${open?"접기":"펼치기"}`;
  btn.dataset.open=open?"1":"0";
}
$("toggleEquipA").addEventListener("click",()=>setEquipOpen("A",$("toggleEquipA").dataset.open!=="1"));
$("toggleEquipB").addEventListener("click",()=>setEquipOpen("B",$("toggleEquipB").dataset.open!=="1"));
setEquipOpen("A",false);
setEquipOpen("B",false);
...
async def openchat(msg: str = ""):
    ...
    if body in ("알림켜기", "알림끄기", "알림상태", "테스트"):
        text = "✅ AION2 v56 서버 정상" if body == "테스트" else "📱 알림 설정은 휴대폰 봇에서 처리됩니다."
