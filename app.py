...
app = FastAPI(title="AION2 Server v28 KairaFix")
...
async def ranking_lookup_smart(body: str):
    ...
    if server_name:
        matched = await search_character_on_server(nickname, server_name)
        if not matched:
            return f"⚠️ {server_name} 서버에서 '{nickname}' 캐릭터를 찾지 못했습니다."
        matched.sort(key=lambda row: int(row.get("combatPower") or 0), reverse=True)
        detail = await load_detail(matched[0], nickname)
        info = detail["info"]
    else:
        candidates = await search_characters_all_servers(nickname)
        ...
        for row in candidates:
            unique[row_server_id(row) or row_server_name(row)] = row
        if len(unique) > 1:
            lines = [f"⚠️ '{nickname}' 캐릭터가 여러 서버에 있습니다.", "", "서버명을 붙여주세요."]
            for row in list(unique.values())[:10]:
                cp = round(int(row.get("combatPower") or 0) / 1000) if row.get("combatPower") else "-"
                lines.append(f"• {row_server_name(row) or row_server_id(row)} · {row.get('className') or '-'} · {cp}")
            return "\n".join(lines)
        detail = await load_detail(candidates[0], nickname)
        info = detail["info"]
    ...
    lines = [header]
    for row in rows[:12]:
        lines += ["", f"▶ {row['dungeonName']}", f"#{row['rank']} · DPS {row['dps']:,}"]
        if row.get("cpTierLabel"):
            lines.append(f"CP 구간 : {row['cpTierLabel']}")
        if row.get("bossName") and row.get("bossName") != "—":
            lines.append(f"보스 : {row['bossName']}")
    return "\n".join(lines)
...
# =========================================================
# Core boss schedule engine (NotMeter-independent)
# =========================================================

# Official/current schedule rules used by the bot.
KAIRA_HOURS = (1, 5, 9, 13, 17, 21)
NAHMA_WEEKDAYS = (4, 6)       # Friday, Sunday (Monday=0)
NAHMA_HOUR = 22
NAHMA_MINUTE = 0
ABYSS_WEEKDAYS = (2, 5)       # Wednesday, Saturday
ABYSS_HOUR = 22
ABYSS_MINUTE = 30
AGRO_INTERVAL_HOURS = 12

# Fallback anchor if the newest maintenance notice title cannot be parsed.
...
def _parse_maintenance_end_from_notice(row):
    """
    Parse titles like:
      9/2(수) 4:30 ~ 7:00 정기점검
      9/2 04:30~07:00 점검
    Returns KST datetime or None.
    """
    ...
    try:
        ...
        # Handles a maintenance window crossing midnight.
        if end <= start:
...
async def latest_maintenance_anchor():
    """
    Uses the newest official notice whose title contains '점검' and includes
    an explicit start~end time. This lets Agro's 12-hour cycle automatically
    shift when a later maintenance window is published.
    """
    now_ts = time.time()
...
def next_agro_from_anchor(anchor, now=None):
    now = now or datetime.now(KST)
    interval = timedelta(hours=AGRO_INTERVAL_HOURS)
...
async def format_all_core_bosses():
    ...
    kaira = _next_daily_hours(KAIRA_HOURS, now)
    nahma = _next_weekly(NAHMA_WEEKDAYS, NAHMA_HOUR, NAHMA_MINUTE, now)
    abyss = _next_weekly(ABYSS_WEEKDAYS, ABYSS_HOUR, ABYSS_MINUTE, now)
...
async def field_boss_lookup(query=None):
    ...
    # Other named field bosses still use the existing cache as a fallback.
    # Core commands above are fully independent of NotMeter.
    try:
...
async def openchat_alerts():
    async with _openchat_alert_lock:
        ...
        targets = [
            ...
            ("감시자 카이라", _next_daily_hours(KAIRA_HOURS, now)),
            ("수호신장 나흐마", _next_weekly(
                NAHMA_WEEKDAYS, NAHMA_HOUR, NAHMA_MINUTE, now
            )),
            ("어비스 보스", _next_weekly(
                ABYSS_WEEKDAYS, ABYSS_HOUR, ABYSS_MINUTE, now
            )),
        ]
...
async def debug_boss_schedule():
    ...
    return {
        ...
        "rules": {
            "agro": "latest maintenance end + every 12h",
            "kaira": "daily 01:00/05:00/09:00/13:00/17:00/21:00",
            "nahma": "Fri/Sun 22:00",
            "abyss": "Wed/Sat 22:30",
        },
    }
