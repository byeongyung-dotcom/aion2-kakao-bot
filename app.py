...
<div class="wrap">
  ...
  <div class="panel section">
    <div class="head">장비 · 아르카나 사진 비교</div>
    <div class="visual-compare">
      <div class="visual-owner a">
        <div class="visual-owner-title" id="visualTitleA">A</div>
        <div class="visual-subhead">장비</div><div class="visual-cards" id="equipVisualA"></div>
        <div class="visual-subhead">아르카나</div><div class="visual-cards" id="arcanaVisualA"></div>
      </div>
      <div class="visual-owner b">
        <div class="visual-owner-title" id="visualTitleB">B</div>
        <div class="visual-subhead">장비</div><div class="visual-cards" id="equipVisualB"></div>
        <div class="visual-subhead">아르카나</div><div class="visual-cards" id="arcanaVisualB"></div>
      </div>
    </div>
  </div>
...
function visualCard(row,kind){
  const name=String(row?.name||"-");
  const icon=String(row?.icon||"").trim();
  const fallback=E(name.slice(0,1)||"?");
  const pic=icon
    ? `<div class="visual-icon"><img src="${E(icon)}" alt="" onerror="this.parentElement.innerHTML='${fallback}'"></div>`
    : `<div class="visual-icon">${fallback}</div>`;
  let meta="";
  if(kind==="equip"){
    const parts=[];
    if(row?.slot) parts.push(row.slot);
    if(row?.enhance!==null && row?.enhance!==undefined && row?.enhance!=="") parts.push(`+${row.enhance}`);
    if(row?.grade) parts.push(row.grade);
    meta=parts.join(" · ");
  }else{
    const parts=[];
    if(row?.level!==null && row?.level!==undefined && row?.level!=="") parts.push(`Lv.${row.level}`);
    if(row?.grade) parts.push(row.grade);
    meta=parts.join(" · ");
  }
  return `<div class="visual-card">${pic}<div><div class="visual-name" title="${E(name)}">${E(name)}</div><div class="visual-meta">${E(meta||"상세 정보")}</div></div></div>`;
}

function renderVisualDetails(a,b){
  [["A",a],["B",b]].forEach(([side,x])=>{
    const equip=$(side==="A"?"equipVisualA":"equipVisualB");
    const arc=$(side==="A"?"arcanaVisualA":"arcanaVisualB");
    const title=$(side==="A"?"visualTitleA":"visualTitleB");
    title.textContent=x?.info?.name||side;
    const er=Array.isArray(x?.equipment)?x.equipment:[];
    const ar=Array.isArray(x?.arcana)?x.arcana:[];
    equip.innerHTML=er.length?er.map(r=>visualCard(r,"equip")).join(""):'<div class="visual-empty">장비 사진 데이터 없음</div>';
    arc.innerHTML=ar.length?ar.map(r=>visualCard(r,"arcana")).join(""):'<div class="visual-empty">아르카나 사진 데이터 없음</div>';
  });
}
...
async def compare_character_data_db_first(nickname: str, server_name: str):
    ...
    equipment = _equipment_rows(full_profile)
    stones = _stone_totals_from_equipment(equipment)
    stats = extract_profile_stats(full_profile)
    ...
    try:
        ...
        if not a.get("ok") or not b.get("ok"):
            ...
            return JSONResponse(
                {
                    "ok": False,
                    "version": "v63-compare-only-db-first",
