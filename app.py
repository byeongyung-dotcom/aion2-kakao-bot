...
app = FastAPI(title="AION2 Server v45 ValidatedAION2Engine")
...
def stat_marginal_efficiency(combined_rows):
    baseline = damage_index_from_rows(combined_rows)["validatedScore"]
    if baseline <= 0:
        return []

    # Only stats that have a validated place in the v45 relative model.
    names = (
        "공격력",
        "피해 증폭", "PVE 피해 증폭", "보스 피해 증폭",
        "치명타 피해 증폭",
        "후방 피해 증폭", "전방 피해 증폭",
    )
    ...
    for name in names:
        ...
        score = damage_index_from_rows(modified)["validatedScore"]
        gain = ((score / baseline) - 1.0) * 100.0
        rows.append({
            "name": name,
            "gainPctPer1": gain,
            "confidence": "high",
    ...
    # Strong experimental facts with rate interpretation rather than raw-stat mapping.
    rows.append({
        "name": "강타 발동률",
        "gainPctPer1": ((1.01 / 1.0) - 1.0) * 100.0,
        "confidence": "high",
        "note": "PvE에서 강타 발동률 +1%p의 단순 기대값",
    })
...
def build_pro_analysis(a, b):
    ...
    efficiency = stat_marginal_efficiency(ca)
    ...
    priorities = []
    for rank, row in enumerate(deficits[:6], start=1):
        stone_shortage = row["targetStone"] - row["mineStone"]
        if stone_shortage > 0:
            action = f"마석 총합 약 {stone_shortage:.2f} 부족"
        else:
            action = "장비/아르카나/스킬/패시브 차이 우선 확인"
        priorities.append({
            "rank": rank,
            **row,
            "action": action,
...
<title>AION2 프로 딜 세팅 비교</title>
<style>
:root{
  --bg:#07101c;--panel:#101a2a;--panel2:#0b1422;--line:#26364e;
  --txt:#f4f7fb;--muted:#9cafc8;--accent:#77a8ff;--good:#6ed69b;
  --bad:#ff7f8b;--gold:#f7cf68;--purple:#b994ff;--cyan:#75d7ff
}
*{box-sizing:border-box}
body{margin:0;background:linear-gradient(180deg,#07101c,#0b1320 50%,#101728);color:var(--txt);font-family:Arial,"Noto Sans KR",sans-serif}
.wrap{max-width:1440px;margin:auto;padding:28px 16px 80px}
.hero,.box{background:rgba(16,26,42,.97);border:1px solid var(--line);border-radius:18px;padding:18px}
h1{margin:0;font-size:30px}.subtitle{color:var(--muted);margin-top:6px;line-height:1.5}
.kicker{font-size:12px;color:var(--accent);font-weight:900;margin-bottom:4px}
...
input,button{padding:11px 12px;border-radius:10px;border:1px solid var(--line);background:#091322;color:var(--txt);font-size:14px}
button{background:var(--accent);color:#07101c;font-weight:900;cursor:pointer;border:none}
.actions{margin-top:12px}.status{font-size:12px;color:var(--muted);margin-top:8px;min-height:18px}
.section{margin-top:16px}.hidden{display:none}.headline{font-size:20px;font-weight:900;margin-bottom:12px}
.profile{display:grid;grid-template-columns:90px 1fr;gap:14px;align-items:center}
.avatar{width:90px;height:90px;border-radius:16px;object-fit:cover;background:#0a1321;border:1px solid var(--line)}
.avatar.fallback{display:flex;align-items:center;justify-content:center;font-size:34px;font-weight:900;color:var(--accent)}
.char-name{font-size:24px;font-weight:900}.meta{color:var(--muted);margin-top:4px}.cp{font-size:38px;font-weight:900;margin-top:7px;color:var(--gold)}
table{width:100%;border-collapse:collapse;background:var(--panel2);border-radius:12px;overflow:hidden}
th,td{padding:11px 10px;border-bottom:1px solid var(--line);font-size:13px}th{color:var(--muted);text-align:left}
td:nth-child(n+2),th:nth-child(n+2){text-align:right}tr:last-child td{border-bottom:none}.plus{color:var(--good);font-weight:900}.minus{color:var(--bad);font-weight:900}
.eqcols{display:grid;grid-template-columns:1fr 1fr;gap:14px}.cardgrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(145px,1fr));gap:10px}
.clickcard,.equip{background:var(--panel2);border:1px solid var(--line);border-radius:13px;padding:11px;cursor:pointer;transition:.15s transform,.15s border-color}
.clickcard:hover,.equip:hover{transform:translateY(-1px);border-color:#496b9f}
.icon{width:58px;height:58px;border-radius:10px;object-fit:cover;background:#0a1321;border:1px solid var(--line)}
.cardname{font-weight:900;margin-top:7px}.tiny{font-size:11px;color:var(--muted);margin-top:3px}
.stone{display:inline-block;padding:4px 7px;border-radius:8px;background:#1a2a44;margin:5px 4px 0 0;font-size:12px}
.badge{display:inline-block;padding:3px 7px;border-radius:999px;background:#17263d;color:#c9dcff;font-size:11px;margin-right:4px}
.notice{padding:12px 14px;border:1px dashed var(--line);border-radius:12px;background:#0a1321;color:var(--muted);line-height:1.5}
.analysis{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.rec{background:var(--panel2);border:1px solid var(--line);border-radius:12px;padding:14px}.rec b{display:block;margin-bottom:6px}
.rank{color:var(--gold);font-weight:900}.calcgrid{display:grid;grid-template-columns:repeat(6,1fr);gap:8px}.field label{display:block;font-size:11px;color:var(--muted);margin-bottom:4px}
.score{font-size:34px;font-weight:900;color:var(--cyan)}.delta{font-size:22px;font-weight:900}.modal{position:fixed;inset:0;background:rgba(0,0,0,.72);display:none;align-items:center;justify-content:center;padding:18px;z-index:99}
.modal.open{display:flex}.modalbox{width:min(760px,100%);max-height:84vh;overflow:auto;background:#101a2a;border:1px solid #395073;border-radius:18px;padding:18px}
.modalhead{display:flex;gap:13px;align-items:center}.close{margin-left:auto;background:#1b2a42;color:#fff;width:auto}.detailopt{padding:8px 0;border-bottom:1px solid var(--line);font-size:13px}
@media(max-width:950px){.grid2,.grid3,.eqcols,.analysis,.calcgrid,.row{grid-template-columns:1fr}}
</style>
</head>
<body>
<div class="wrap">
  <div class="hero">
    <div class="kicker">AION2 DAMAGE ENGINE</div>
    <h1>AION2 프로 딜 분석 · 성장 로드맵</h1>
    <div class="subtitle">장비·마석·아르카나·스킬·패시브·랭킹까지 함께 보고, 상대와의 전투력/딜 차이가 어디서 생기는지 분석합니다.</div>
    <div class="grid2" style="margin-top:18px">
      <div class="box"><b>내 캐릭터</b><div class="row"><input id="na" value="윤이"><input id="sa" value="지켈"></div><div class="status" id="sta"></div></div>
      <div class="box"><b>비교 캐릭터</b><div class="row"><input id="nb" placeholder="캐릭터명"><input id="sb" value="지켈"></div><div class="status" id="stb"></div></div>
    </div>
    <div class="actions"><button id="go">프로 비교 시작</button></div>
  </div>

  <div id="result" class="hidden">
    <div class="grid2 section"><div class="box" id="ca"></div><div class="box" id="cb"></div></div>

    <div class="box section">
      <div class="headline">핵심 공격 스탯 + 마석 합산</div>
      <table><thead><tr><th>항목</th><th id="ha">A 기본</th><th>A 마석</th><th>A 합산</th><th id="hb">B 기본</th><th>B 마석</th><th>B 합산</th><th>차이</th></tr></thead><tbody id="combined"></tbody></table>
    </div>

    <div class="box section">
      <div class="headline">마석 상세 비교</div>
      <table><thead><tr><th>마석</th><th>A 개수 / 총합 / 평균</th><th>B 개수 / 총합 / 평균</th><th>총합 차이</th></tr></thead><tbody id="stones"></tbody></table>
    </div>

    <div class="box section">
      <div class="headline">검증 딜지수 계산</div>
      <div class="notice">검증도가 높은 관계만 딜지수에 반영합니다. 무피·관통·완벽·다단히트처럼 스킬별 적용식이 완전히 확정되지 않은 항목은 별도 분석하며 임의로 최종 배율에 넣지 않습니다.</div>
      <div class="calcgrid" style="margin-top:12px">
        <div class="field"><label>보스 피해 내성 %</label><input id="resist" type="number" value="0" step="0.1"></div>
        <div class="field"><label>치명 발동률 %</label><input id="critRate" type="number" value="50"></div>
        <div class="field"><label>강타 발동률 %</label><input id="hardRate" type="number" value="0"></div>
        <div class="field"><label>후방 공격 비율 %</label><input id="backRatio" type="number" value="100"></div>
        <div class="field"><label>전방 공격 비율 %</label><input id="frontRatio" type="number" value="0"></div>
        <div class="field"><label>스킬 계수</label><input id="skillCoeff" type="number" value="1" step="0.01"></div>
      </div>
      <div class="grid2" style="margin-top:14px">
        <div class="rec"><b id="scoreNameA">A 딜지수</b><div class="score" id="scoreA">-</div></div>
        <div class="rec"><b id="scoreNameB">B 딜지수</b><div class="score" id="scoreB">-</div></div>
      </div>
      <div class="rec" style="margin-top:10px"><b>예상 차이</b><div class="delta" id="scoreDelta">-</div></div>
    </div>

    <div class="box section">
      <div class="headline">계산 신뢰도</div>
      <div class="analysis">
        <div class="rec"><b>높은 신뢰도</b><div class="tiny">피증 합연산 · 후/전피 방향 배율 · 치명 150%+치피 · PvE 강타 2배 · 명중 게이트</div></div>
        <div class="rec"><b>분리 분석</b><div class="tiny">무피 · 관통 · 완벽 · 다단히트는 전체 최종배율로 억지 계산하지 않음</div></div>
        <div class="rec"><b>스킬 계산</b><div class="tiny">스킬별 고정 피해/레벨/쿨/특화가 실제 프로필에서 확인될 때만 액티브 효율에 반영</div></div>
      </div>
    </div>

    <div class="box section"><div class="headline">전문 성장 피드백</div><div class="analysis" id="analysis"></div></div>
    <div class="box section"><div class="headline">전투력 상승 요인 비교</div><table><thead><tr><th>성장 요소</th><th>A</th><th>B</th><th>차이</th></tr></thead><tbody id="growth"></tbody></table></div>
    <div class="box section"><div class="headline">NotMeter 랭킹 비교</div><div class="grid2"><div id="ranka"></div><div id="rankb"></div></div></div>

    <div class="box section"><div class="headline">전체 장비 · 클릭해서 상세보기</div><div class="eqcols"><div><b id="eqha">A</b><div id="eqa" class="cardgrid" style="margin-top:10px"></div></div><div><b id="eqhb">B</b><div id="eqb" class="cardgrid" style="margin-top:10px"></div></div></div></div>

    <div class="box section"><div class="headline">아르카나 · 클릭해서 정보보기</div><div class="grid2"><div><b id="archa">A</b><div id="arca" class="cardgrid" style="margin-top:10px"></div></div><div><b id="archb">B</b><div id="arcb" class="cardgrid" style="margin-top:10px"></div></div></div></div>

    <div class="box section"><div class="headline">스킬 레벨</div><div class="grid2"><div><b id="skha">A</b><div id="ska" class="cardgrid" style="margin-top:10px"></div></div><div><b id="skhb">B</b><div id="skb" class="cardgrid" style="margin-top:10px"></div></div></div></div>

    <div class="box section"><div class="headline">패시브 레벨</div><div class="grid2"><div><b id="psha">A</b><div id="psa" class="cardgrid" style="margin-top:10px"></div></div><div><b id="pshb">B</b><div id="psb" class="cardgrid" style="margin-top:10px"></div></div></div></div>
  </div>
</div>

<div id="modal" class="modal"><div class="modalbox"><div class="modalhead"><img id="modalIcon" class="icon"><div><div id="modalName" class="char-name"></div><div id="modalMeta" class="meta"></div></div><button class="close" onclick="closeModal()">닫기</button></div><div id="modalBody" style="margin-top:14px"></div></div></div>
...
function avatar(i){const src=i.profileImage||"";if(src)return `<img class="avatar" src="${E(src)}"><div class="avatar fallback" style="display:none">${E((i.name||'?').slice(0,1))}</div>`;return `<div class="avatar fallback">${E((i.name||'?').slice(0,1))}</div>`}
function pcard(x){const i=x.info||{};return `<div class="profile"><div>${avatar(i)}</div><div><div class="char-name">${E(i.name)}</div><div class="meta">${E(i.server)} · ${E(i.job)} · Lv.${F(i.level)}</div><div class="cp">${Math.round(Number(i.combatPower||0)/1000)}</div><div class="meta">전투력</div></div></div><div class="status">${x.profileAvailable?'상세 프로필 연결됨':'기본정보만'}</div>`}
function cmap(x){const m={};(x.combinedOffense||[]).forEach(v=>m[v.name]=v);return m}
function smap(x){const m={};(x.magicStoneTotals||[]).forEach(v=>m[v.name]=v);return m}
function cardHtml(x,type){return `<div class="clickcard" onclick='openModal(${JSON.stringify(x).replace(/'/g,"&#39;")})'>${x.icon?`<img class="icon" src="${E(x.icon)}">`:`<div class="icon"></div>`}<div class="cardname">${E(x.name||"-")}</div><div class="tiny">${E(type)} ${x.level!=null?`· Lv.${E(x.level)}`:""} ${x.grade?`· ${E(x.grade)}`:""}</div></div>`}
function equipmentHtml(x){return `<div class="equip" onclick='openModal(${JSON.stringify(x).replace(/'/g,"&#39;")})'>${x.icon?`<img class="icon" src="${E(x.icon)}">`:`<div class="icon"></div>`}<div class="cardname">${x.enhance!=null?`+${E(x.enhance)} `:""}${E(x.name)}</div><div class="tiny">${E(x.slot)} ${x.grade?`· ${E(x.grade)}`:""}</div><div>${(x.magicStones||[]).map(s=>`<span class="stone">${E(s.name)} ${E(s.value)}</span>`).join("")}</div></div>`}
function openModal(x){modal.classList.add("open");modalIcon.src=x.icon||"";modalName.textContent=x.name||"-";modalMeta.textContent=[x.slot,x.grade,x.level!=null?`Lv.${x.level}`:""].filter(Boolean).join(" · ");let html=x.description?`<div class="notice">${E(x.description)}</div>`:"";(x.options||[]).forEach(o=>html+=`<div class="detailopt"><b>${E(o.name)}</b> ${E(o.value??"")}<div class="tiny">${E(o.description||"")}</div></div>`);(x.magicStones||[]).forEach(s=>html+=`<div class="detailopt">💎 <b>${E(s.name)}</b> ${E(s.value)}</div>`);modalBody.innerHTML=html||`<div class="notice">추가 상세정보 없음</div>`}
function closeModal(){modal.classList.remove("open")}
function renderCombined(a,b){const ma=cmap(a),mb=cmap(b);const order=["공격력","추가 공격력","최소 공격력","최대 공격력","PVE 공격력","보스 공격력","공격력 증가율","피해 증폭","무기 피해 증폭","PVE 피해 증폭","보스 피해 증폭","치명타 피해 증폭","전방 피해 증폭","후방 피해 증폭","치명타","명중","강타","완벽","관통","공격 속도","시전 속도"];combined.innerHTML=order.filter(n=>ma[n]||mb[n]).map(n=>{const x=ma[n]||{baseValue:0,stoneTotal:0,combinedValue:0},y=mb[n]||{baseValue:0,stoneTotal:0,combinedValue:0};const xv=x.combinedValue??x.baseValue,yv=y.combinedValue??y.baseValue;return `<tr><td>${E(n)}</td><td>${F(x.baseValue)}</td><td>${F(x.stoneTotal)}</td><td>${F(xv)}</td><td>${F(y.baseValue)}</td><td>${F(y.stoneTotal)}</td><td>${F(yv)}</td><td>${D(xv,yv)}</td></tr>`}).join("")}
function renderStones(a,b){const sa=smap(a),sb=smap(b),names=[...new Set([...Object.keys(sa),...Object.keys(sb)])];stones.innerHTML=names.map(n=>{const x=sa[n]||{count:0,total:0,average:0},y=sb[n]||{count:0,total:0,average:0};return `<tr><td>${E(n)}</td><td>${x.count} / ${F(x.total)} / ${F(x.average)}</td><td>${y.count} / ${F(y.total)} / ${F(y.average)}</td><td>${D(x.total,y.total)}</td></tr>`}).join("")}
async function calc(){if(!DATA)return;const p={criticalRate:+critRate.value,hardHitRate:+hardRate.value,backRatio:+backRatio.value,frontRatio:+frontRatio.value,bossResistance:+resist.value,skillCoefficient:+skillCoeff.value};const one=async rows=>{const r=await fetch("/api/damage-index",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...p,combinedOffense:rows})});return r.json()};const [a,b]=await Promise.all([one(DATA.a.combinedOffense),one(DATA.b.combinedOffense)]);scoreA.textContent=F(a.score);scoreB.textContent=F(b.score);const delta=(a.score/b.score-1)*100;scoreDelta.innerHTML=`<span class="${delta>=0?'plus':'minus'}">${delta>=0?'+':''}${delta.toFixed(2)}%</span>`}
function renderAnalysis(p){let html="";(p.priorities||[]).forEach(r=>html+=`<div class="rec"><span class="rank">#${r.rank}</span><b>${E(r.name)}</b><div class="tiny">상대 대비 ${F(r.gap)} 부족 · ${E(r.action)}</div></div>`);(p.swapCandidates||[]).forEach(r=>html+=`<div class="rec"><b>마석 교체 후보</b><div class="tiny">${E(r.from)} 초과 ${F(r.fromExcess)} → ${E(r.to)} 부족 ${F(r.toShortage)}</div></div>`);(p.marginalEfficiency||[]).slice(0,3).forEach(r=>html+=`<div class="rec"><b>+1 효율 ${E(r.name)}</b><div class="tiny">현재 세팅 기준 기대딜 약 +${Number(r.gainPctPer1).toFixed(3)}%</div></div>`);analysis.innerHTML=html||`<div class="notice">비교 가능한 공격 스탯 부족</div>`}
function renderGrowth(p){growth.innerHTML=(p.growthFactors||[]).map(r=>`<tr><td>${E(r.name)}</td><td>${F(r.mine)}</td><td>${F(r.target)}</td><td>${D(r.mine,r.target)}</td></tr>`).join("")}
function renderRank(el,rows,name){el.innerHTML=`<b>${E(name)}</b>`+((rows||[]).map(r=>`<div class="rec" style="margin-top:8px"><b>${E(r.dungeonName||"-")}</b><div class="tiny">#${E(r.rank)} · DPS ${F(r.dps)} ${r.cpTierLabel?`· ${E(r.cpTierLabel)}`:""}</div></div>`).join("")||`<div class="notice" style="margin-top:8px">공개 랭킹 기록 없음</div>`)}
function render(d){DATA=d;const a=d.a,b=d.b,p=d.proAnalysis||{};ca.innerHTML=pcard(a);cb.innerHTML=pcard(b);ha.textContent=eqha.textContent=archa.textContent=skha.textContent=psha.textContent=a.info.name;hb.textContent=eqhb.textContent=archb.textContent=skhb.textContent=pshb.textContent=b.info.name;scoreNameA.textContent=`${a.info.name} 딜지수`;scoreNameB.textContent=`${b.info.name} 딜지수`;renderCombined(a,b);renderStones(a,b);eqa.innerHTML=(a.equipment||[]).map(equipmentHtml).join("");eqb.innerHTML=(b.equipment||[]).map(equipmentHtml).join("");arca.innerHTML=(a.arcana||[]).map(x=>cardHtml(x,"아르카나")).join("")||`<div class="notice">아르카나 데이터 없음</div>`;arcb.innerHTML=(b.arcana||[]).map(x=>cardHtml(x,"아르카나")).join("")||`<div class="notice">아르카나 데이터 없음</div>`;ska.innerHTML=(a.skills||[]).map(x=>cardHtml(x,"스킬")).join("");skb.innerHTML=(b.skills||[]).map(x=>cardHtml(x,"스킬")).join("");psa.innerHTML=(a.passives||[]).map(x=>cardHtml(x,"패시브")).join("");psb.innerHTML=(b.passives||[]).map(x=>cardHtml(x,"패시브")).join("");renderAnalysis(p);renderGrowth(p);renderRank(ranka,p.rankingA,a.info.name);renderRank(rankb,p.rankingB,b.info.name);result.classList.remove("hidden");calc()}
async function compare(){sta.textContent=stb.textContent="장비·마석·아르카나·스킬·랭킹 분석 중...";const q=new URLSearchParams({name_a:na.value.trim(),server_a:sa.value.trim(),name_b:nb.value.trim(),server_b:sb.value.trim()});try{const r=await fetch(`/api/compare?${q}`);const d=await r.json();if(!d.ok){sta.textContent="내 캐릭터 또는 상대 조회 실패";stb.textContent="조회 실패";return}sta.textContent=d.a.profileAvailable?"상세 프로필 연결됨":"기본정보만";stb.textContent=d.b.profileAvailable?"상세 프로필 연결됨":"기본정보만";render(d)}catch(e){sta.textContent=stb.textContent="비교 서버 오류"}}
go.addEventListener("click",compare);[resist,critRate,hardRate,backRatio,frontRatio,skillCoeff].forEach(x=>x.addEventListener("input",calc));
</script>
</body>
</html>"""
...
async def api_compare_health():
    return {
        "ok": True,
        "version": "v45-validated-aion2-engine",
        "cors": True,
    }
...
async def api_damage_index(request: Request):
    try:
        ...
        result = damage_index_from_rows(
            ...
            back_ratio=payload.get("backRatio", 100),
...
async def openchat(msg: str = ""):
    ...
    if body in ("알림켜기", "알림끄기", "알림상태", "테스트"):
        text = "✅ AION2 v45 서버 정상" if body == "테스트" else "📱 알림 설정은 휴대폰 봇에서 처리됩니다."
