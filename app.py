...
def _normalize_openchat_delivery(raw):
    ...
    if isinstance(raw.get("sentKeys"), list):
        out["sentKeys"] = [str(x) for x in raw["sentKeys"] if str(x)][-1000:]
    return out
...
async def openchat_alerts(room: str = ""):
    ...
    async with _openchat_alert_lock:
        ...
        # Claim each alert key on the server before returning it.
        # This guarantees one delivery per room even if several phone timers poll at once.
        ...
        delivery["sentKeys"] = list(sent_keys)[-1000:]
        delivery["bossSent"] = []
        delivery["inflight"] = {}
