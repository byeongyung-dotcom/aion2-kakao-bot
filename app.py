...
async def party_image(section_name: str):
    ...
    # 이전에 이미지가 보이던 방식 우선: Google Sheets의 해당 고정 범위를
    # 직접 이미지로 요청한다. 실패하면 서버 PNG 렌더링으로만 보완한다.
    try:
        client = await get_http_client()
        image_attempts = []
        gid = str(cfg.get("gid") or "").strip()
        ...
        if gid:
            image_attempts.append({"format": "png", "gid": gid, "range": a1})
        if sheet_name:
            image_attempts.append({"format": "png", "sheet": sheet_name, "range": a1})

        export_url = f"https://docs.google.com/spreadsheets/d/{PARTY_SHEET_ID}/export"
        for params in image_attempts:
            try:
                res = await client.get(
                    export_url,
                    params=params,
                ...
                if 200 <= res.status_code < 300:
                    ...
                    if "image" in ctype and res.content:
                        return Response(
                            content=res.content,
        ...
        # Google 직접 이미지가 안 될 때만 기존 CSV 데이터를 읽어서 PNG로 생성.
        ...
        try:
            ...
            col_widths = [145, 225, 150, 185, 245]
            row_h, title_h, margin = 52, 68, 18
            width = sum(col_widths) + margin * 2
            ...
            for r_idx, row in enumerate(rows):
                ...
                party_header = ("1파티" in joined) or ("2파티" in joined)
                summary = r_idx in (0, 1)
                ...
                for c, cell in enumerate(row):
                    ...
                    if value:
                        ...
                        th = bbox[3] - bbox[1]
                        draw.text((x + 8, y + max(6, (row_h - th) / 2 - 2)), value, fill=text_fill, font=font)
                    x += w
            ...
            # 최후 호환용. 브라우저에서는 볼 수 있지만 카카오 미리보기는 PNG가 우선이다.
            svg = _party_svg(display_name, rows)
            return Response(
