...
def format_character_from_data(info, stones):
    ...
    lines = [
        f"⚔️ {name}",
        f"직업 · {job}" if job else None,
        f"전투력 · {cp_short:,}" if cp_short else None,
    ]

    # Kakao preview requires an actual URL in a plain-text message.
    # Keep it on the final line only, so the visible character text stays clean.
    if server:
        # Change the preview URL whenever combat power changes so Kakao does
        # not keep showing an older OpenGraph card.
...
async def _fresh_official_character(nickname, server_name=None):
    """Fast live NC lookup for direct character commands.

    For a server-specific command such as !윤이지켈, hit the official search
    endpoint once for that server, then load only that one character detail.
    Local 120/180s character caches are explicitly discarded first.
    """
    nickname = str(nickname or "").strip()
    server_name = str(server_name or "").strip() or None
    if not nickname:
    ...
    _drop_character_live_caches(nickname, server_name)

    # Direct server lookup: one search request + one detail request only.
    if server_name:
        ...
        for item in (data.get("list") or []):
            ...
            if item_name.casefold() != nickname.casefold() or item_sid != int(sid):
                continue
            cid = unquote(str(item.get("characterId") or item.get("charId") or item.get("id") or "").strip())
            if not cid:
                continue
            row = {
                "name": item_name,
                ...
                "characterId": cid,
                "officialUrl": f"{OFFICIAL_CHARACTER_BASE}/ko-kr/characters/{item_sid}/{quote(cid, safe='')}",
            }
            break
        if not row:
            return None

        # Clear the exact detail cache immediately before the one detail call.
        _drop_character_live_caches(nickname, server_name, [row])
        try:
            info = await asyncio.wait_for(official_load_detail(row), timeout=5.5)
        except Exception:
            return None
    ...
    # No server supplied: keep the existing all-server official resolver.
    rows = await official_search_characters(nickname, None)
    if not rows:
        return None
    _drop_character_live_caches(nickname, None, rows)
...
# redeploy may clear /tmp unless BOSS_SCHEDULE_OVERRIDE_FILE points to persistent storage.
BOSS_SCHEDULE_OVERRIDE_FILE = Path(
    os.getenv("BOSS_SCHEDULE_OVERRIDE_FILE", "/tmp/aion2_boss_schedule_overrides.json")
)
...
def _load_boss_schedule_overrides():
    ...
    try:
        if not BOSS_SCHEDULE_OVERRIDE_FILE.exists():
            _BOSS_SCHEDULE_OVERRIDE_CACHE = {}
            return {}
        raw = json.loads(BOSS_SCHEDULE_OVERRIDE_FILE.read_text(encoding="utf-8"))
...
def _save_boss_schedule_overrides(data):
    global _BOSS_SCHEDULE_OVERRIDE_CACHE
    try:
        # Update memory FIRST so the very next /openchat/alerts request on this
        # process sees the new value immediately.
        _BOSS_SCHEDULE_OVERRIDE_CACHE = json.loads(json.dumps(data, ensure_ascii=False))
        BOSS_SCHEDULE_OVERRIDE_FILE.parent.mkdir(parents=True, exist_ok=True)
        tmp = BOSS_SCHEDULE_OVERRIDE_FILE.with_suffix(".tmp")
        tmp.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
        tmp.replace(BOSS_SCHEDULE_OVERRIDE_FILE)
        return True
    except Exception:
        return False
...
def _get_schedule_alert_leads(name):
    key = _schedule_key(name)
    if not key:
        return list(DEFAULT_SCHEDULE_ALERT_LEADS)
    data = _load_boss_schedule_overrides()
    rows = data.get("alertLeads") or {}
    leads = rows.get(key) if isinstance(rows, dict) else None
...
def _clear_schedule_delivery_keys(name):
    """Drop stale sent/inflight keys for one schedule after its lead-time changes."""
    canonical = {
        "agro": "정령왕 아그로",
        "kaira": "감시자 카이라",
        "nahma": "수호신장 나흐마",
    ...
    try:
        ...
        changed = False
        for delivery in (state.get("deliveries") or {}).values():
            if not isinstance(delivery, dict):
                continue
            old_sent = list(delivery.get("bossSent") or [])
            new_sent = [k for k in old_sent if not str(k).startswith(prefix)]
            if new_sent != old_sent:
                delivery["bossSent"] = new_sent
                changed = True
            old_inflight = dict(delivery.get("inflight") or {})
            new_inflight = {k: v for k, v in old_inflight.items() if not str(k).startswith(prefix)}
            if new_inflight != old_inflight:
                delivery["inflight"] = new_inflight
                changed = True
        if changed:
...
def _set_schedule_alert_leads(name, leads):
    key = _schedule_key(name)
    if not key:
        return False
    cleaned = []
    ...
    if not cleaned:
        return False
    data = _load_boss_schedule_overrides()
    rows = data.get("alertLeads")
    if not isinstance(rows, dict):
        rows = {}
    rows[key] = sorted(cleaned, reverse=True)
    data["alertLeads"] = rows
    ok = _save_boss_schedule_overrides(data)
    if ok:
        _clear_schedule_delivery_keys(name)
    return ok


def _manual_alert_lead_command(name, value_text):
    ...
    if not _set_schedule_alert_leads(name, nums):
...
def _manual_agro_anchor_for_source(source_id):
    data = _load_boss_schedule_overrides()
    row = data.get("agro") or {}
    if str(row.get("sourceId") or "") != str(source_id or ""):
        return None
    try:
        value = datetime.fromisoformat(str(row.get("anchor") or ""))
...
def _set_manual_schedule(name, hour, minute):
    ...
    if key == "아그로":
        ...
        data["agro"] = {"anchor": anchor.isoformat(), "sourceId": source_id}
...
async def latest_maintenance_anchor():
    ...
    if cached is not None and now_ts - cached_ts < 300:
        # A manual correction must take effect immediately even while the
        # maintenance notice cache is still warm.
        manual_anchor = _manual_agro_anchor_for_source(
            _maintenance_anchor_cache.get("sourceId")
        )
        return manual_anchor if manual_anchor is not None else cached
    ...
    if parsed_rows:
        ...
        manual_anchor = _manual_agro_anchor_for_source(chosen["id"])
    ...
    _maintenance_anchor_cache["value"] = anchor
    _maintenance_anchor_cache["ts"] = now_ts
    manual_anchor = _manual_agro_anchor_for_source(_maintenance_anchor_cache.get("sourceId"))
    return manual_anchor if manual_anchor is not None else anchor
...
async def _manual_schedule_command(name, clock_text):
    ...
    # Agro needs the current maintenance source id before binding the manual correction.
    if name == "아그로":
        try:
...
def _cached_alert_agro_anchor():
    ...
    manual = _manual_agro_anchor_for_source(source_id)
    if manual is not None:
        return manual
    return cached if cached is not None else AGRO_FALLBACK_ANCHOR
...
def _default_openchat_alert_state():
    return {
        # Global switch is kept for older phone-bot scripts.
        "enabled": True,
        # Per-room ON/OFF switches.
        "rooms": {},
        # Per-room delivery state. Each room gets each alert once.
        "deliveries": {},
    }
...
def _normalize_openchat_delivery(raw):
    ...
    if isinstance(raw.get("boardSent"), list):
        out["boardSent"] = [str(x) for x in raw["boardSent"]][-500:]
    if isinstance(raw.get("bossSent"), list):
        out["bossSent"] = [str(x) for x in raw["bossSent"]][-100:]
    if isinstance(raw.get("inflight"), dict):
        clean = {}
        for k, v in raw["inflight"].items():
            try:
                clean[str(k)] = float(v)
            except Exception:
                pass
    ...
    if isinstance(raw.get("testQueue"), list):
        out["testQueue"] = [
            x for x in raw["testQueue"][-20:]
            if isinstance(x, dict)
        ]
    if isinstance(raw.get("boardPending"), list):
        out["boardPending"] = [
            x for x in raw["boardPending"][-100:]
            if isinstance(x, dict)
        ]
    return out
    ...
    try:
        ...
        if isinstance(raw.get("deliveries"), dict):
            state["deliveries"] = {
                ...
                for k, v in raw["deliveries"].items()
                if str(k).strip()
            }

        # Backward migration: old global lastSeen/boardSent/bossSent becomes the
        # legacy no-room delivery state only. It is NOT shared by named rooms.
...
async def openchat_alerts(room: str = ""):
    """Retry-safe polling endpoint.

    Server never consumes scheduled/test alerts on GET. The same key remains
    available during its delivery window; the phone performs the final
    one-time de-duplication after a successful Kakao send.
    """
    room_key = _openchat_room_key(room)
    ...
    async with _openchat_alert_lock:
        ...
        for item_type, name, target in targets:
            ...
            event_id = f"{name}|{int(target.timestamp())}"
            for lead in _get_schedule_alert_leads(name):
        ...
        # Old server-side delivery claims are no longer authoritative.
        delivery["bossSent"] = []
        delivery["inflight"] = {}
        ...
        if items:
            return {"ok": True, "enabled": True, "room": room_key, "baseline": first_run, "items": items}
    ...
    async with _openchat_alert_lock:
        ...
        return {
            ...
            "room": room_key,
            "baseline": first_run,
            "items": [x["item"] for x in pending],
        }


@app.get("/openchat/alerts/ack")
async def openchat_alert_ack(room: str = "", key: str = ""):
    return {"ok": True, "room": _openchat_room_key(room), "key": str(key or "").strip()}
...
async def _format_openchat_alert_diagnostic(room: str = ""):
    ...
    sent = set(str(x) for x in delivery.get("bossSent", []))
    ...
    for item_type, name, target in targets:
        ...
        minutes = (target - now).total_seconds() / 60.0
        leads = _get_schedule_alert_leads(name)
        event_id = f"{name}|{int(target.timestamp())}"
...
async def openchat(msg: str = "", room: str = ""):
    ...
    if body == "알림진단":
        try:
            text = await asyncio.wait_for(_format_openchat_alert_diagnostic(room), timeout=12.0)
    ...
    if m_alert_lead and ("분" in body or "알림" in body):
        result = _manual_alert_lead_command(m_alert_lead.group(1), m_alert_lead.group(2))
        return PlainTextResponse(result, media_type="text/plain; charset=utf-8")
