...
app = FastAPI(title="AION2 Server v48 FinalIntegrated")
...
<div class="wrap">
  <div class="hero">
    ...
    <div class="sub">미리보기용 UI · 실제 데이터 연결 전</div>
    <div class="search">
  ...
  <div class="panel section">
    ...
    <div class="note">같은 직업이면 실제 패시브 이름을 1:1로 맞춰 레벨 차이를 표시. 다른 직업이면 각 직업의 핵심 패시브 투자 수준을 나란히 비교.</div>
  ...
  <div class="panel section">
    ...
    <div class="note">같은 직업이면 실제 스킬명을 1:1로 맞춰 레벨 차이를 표시. 최종 분석에서는 레벨 차이가 전체 사이클 DPS에 주는 영향까지 연결.</div>
    <table style="margin-top:10px">
  ...
  <div class="panel section">
    <div class="head">전체 스킬 / 패시브</div>
    <div class="split">
      <div class="owner a">
        <div class="ownerhead" id="skillHeadA"></div>
        <div class="cards" id="skillsA"></div>
      </div>
      <div class="owner b">
        <div class="ownerhead" id="skillHeadB"></div>
        <div class="cards" id="skillsB"></div>
      </div>
    </div>
  </div>
</div>
...
const diff=(a,b)=>{
  ...
  return `<span class="${d>0?'good':d<0?'bad':''}">${d>0?'+':''}${F(d)}</span>`;
};

function profileCard(x, side){
  const i=x.info||{};
  const p=(DATA?.proAnalysis||{})[side==='a'?'jobProfileA':'jobProfileB']||{};
  const initials=(i.name||'?').slice(0,1);
  return `<div class="char">
    ${i.profileImage?`<img class="avatar" src="${E(i.profileImage)}" style="object-fit:cover">`:`<div class="avatar">${E(initials)}</div>`}
    <div>
      ...
      <div class="badges">
        <span class="badge">${E(p.directionLabel||'포지션 수동')}</span>
        ${p.frontRatio?`<span class="badge">전방 ${F(p.frontRatio)}%</span>`:''}
        ${p.backRatio?`<span class="badge">후방 ${F(p.backRatio)}%</span>`:''}
      </div>
    </div>
  </div>`;
}

function mapCombined(x){const m={};(x.combinedOffense||[]).forEach(v=>m[v.name]=v);return m}

function renderAttackStats(a,b){
  const ma=mapCombined(a), mb=mapCombined(b);
  const groups=[
  ...
  for(const [label,names] of groups){
    const visible=names.filter(n=>ma[n]||mb[n]);
    if(!visible.length)continue;
    html+=`<tr class="group"><td colspan="4">${E(label)}</td></tr>`;
    for(const n of visible){
      const x=ma[n]||{combinedValue:0,baseValue:0};
      const y=mb[n]||{combinedValue:0,baseValue:0};
      const xv=x.combinedValue??x.baseValue??0;
      const yv=y.combinedValue??y.baseValue??0;
      const suffix=(x.unit==="percent"||y.unit==="percent")?"%":"";
      html+=`<tr><td>${E(n)}</td><td>${F(xv)}${suffix}</td><td>${F(yv)}${suffix}</td><td>${diff(xv,yv)}${suffix}</td></tr>`;
    }
  }
  attackStats.innerHTML=html||`<tr><td colspan="4">공격 스탯 데이터 없음</td></tr>`;
}

function renderGaps(d){
  const p=d.proAnalysis||{};
  const cpGap=Number(d.a?.info?.combatPower||0)-Number(d.b?.info?.combatPower||0);
  const first=(p.priorities||[])[0];
  gapKpis.innerHTML=`
    <div class="kpi"><div class="label">전투력 차이</div><div class="value ${cpGap>=0?'good':'bad'}">${cpGap>=0?'+':''}${Math.round(cpGap/1000)}K</div></div>
    <div class="kpi"><div class="label">가장 큰 검증 격차</div><div class="value">${E(first?.name||'-')}</div></div>
    <div class="kpi"><div class="label">예상 회복 딜</div><div class="value">${first?`+${Number(first.expectedRecoveryPct||0).toFixed(2)}%`:'-'}</div></div>
    <div class="kpi"><div class="label">포지션</div><div class="value" style="font-size:17px">${E(p.jobProfileA?.directionLabel||'-')}</div></div>`;
}

function renderPriorities(p){
  const rows=p.priorities||[];
  priorityCards.innerHTML=rows.slice(0,3).map(r=>`
    <div class="pcard">
      <div class="rank">#${E(r.rank)} ${E(r.name)}</div>
      <div class="pct">+${Number(r.expectedRecoveryPct||0).toFixed(2)}%</div>
      <div class="tiny">${E(r.action||'')}</div>
    </div>`).join("")||`<div class="note">검증 가능한 공격 격차가 크지 않음</div>`;

  const conditional=(p.conditionalChecks||[]).slice(0,4);
  priorityNote.innerHTML=conditional.length
    ? `조건부 확인: ${conditional.map(x=>E(x.name)).join(" · ")} — 발동률/보스 조건이 필요한 항목은 단순 수치차만으로 우선순위를 정하지 않음.`
    : `수치 차이만으로 순위를 정하지 않고 현재 검증 가능한 딜 회복량 기준으로 정렬.`;
}

function openDetail(x){
  m1.textContent=x.name||"-";
  m2.textContent=[x.slot,x.grade,x.level!=null?`Lv.${x.level}`:""].filter(Boolean).join(" · ");
  let body=x.description||"";
  if(x.magicStones?.length){
    body += (body?"\n\n":"") + x.magicStones.map(s=>`💎 ${s.name} ${s.value??''}`).join("\n");
  }
  if(x.options?.length){
    body += (body?"\n\n":"") + x.options.map(o=>`${o.name} ${o.value??''}`).join("\n");
  }
  m3.textContent=body||"추가 상세정보 없음";
  modal.classList.add("show");
}
function hide(){modal.classList.remove("show")}

function cardItem(x, type){
  const obj=encodeURIComponent(JSON.stringify(x));
  return `<div class="card" data-detail="${obj}">
    <div class="rowcard">
      ${x.icon?`<img class="icon" src="${E(x.icon)}" style="object-fit:cover">`:`<div class="icon">${type==="장비"?"⚔️":type==="아르카나"?"✦":"◆"}</div>`}
      <div>
        <div class="ctitle">${E(x.name||'-')}</div>
        <div class="tiny">${E(type)} ${x.level!=null?`· Lv.${E(x.level)}`:""} ${x.enhance!=null?`· +${E(x.enhance)}`:""}</div>
      </div>
    </div>
  </div>`;
}
...
function wireCards(){
  document.querySelectorAll("[data-detail]").forEach(el=>{
    el.onclick=()=>openDetail(JSON.parse(decodeURIComponent(el.dataset.detail)));
  });
}

function renderEquipment(a,b){
  eqHeadA.textContent=`A · ${a.info?.name||'-'} · ${a.info?.job||'-'}`;
  eqHeadB.textContent=`B · ${b.info?.name||'-'} · ${b.info?.job||'-'}`;
  equipA.innerHTML=(a.equipment||[]).map(x=>cardItem(x,"장비")).join("")||`<div class="note">장비 데이터 없음</div>`;
  equipB.innerHTML=(b.equipment||[]).map(x=>cardItem(x,"장비")).join("")||`<div class="note">장비 데이터 없음</div>`;

  arcHeadA.textContent=`A · ${a.info?.name||'-'}`;
  arcHeadB.textContent=`B · ${b.info?.name||'-'}`;
  arcA.innerHTML=(a.arcana||[]).map(x=>cardItem(x,"아르카나")).join("")||`<div class="note">아르카나 데이터 없음</div>`;
  arcB.innerHTML=(b.arcana||[]).map(x=>cardItem(x,"아르카나")).join("")||`<div class="note">아르카나 데이터 없음</div>`;
}

function levelMap(rows){
  const m={};
  (rows||[]).forEach(x=>{ if(x?.name) m[x.name]=x; });
  return m;
}
function avgLevel(rows){
  const vals=(rows||[]).map(x=>Number(x.level)).filter(Number.isFinite);
  return vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0;
}

function renderPassives(a,b){
  const sameJob=(a.info?.job||"")===(b.info?.job||"");
  passiveTitle.textContent=sameJob?`${a.info?.job||''} 패시브 1:1 비교`:"직업별 핵심 고레벨 패시브";
  passiveAName.textContent=a.info?.name||"A";
  passiveBName.textContent=b.info?.name||"B";

  const ma=levelMap(a.passives), mb=levelMap(b.passives);
  let names=sameJob?[...new Set([...Object.keys(ma),...Object.keys(mb)])]:[];

  if(!sameJob){
    const ar=[...(a.passives||[])].filter(x=>Number(x.level)>=20).sort((x,y)=>Number(y.level)-Number(x.level));
    const br=[...(b.passives||[])].filter(x=>Number(x.level)>=20).sort((x,y)=>Number(y.level)-Number(x.level));
    const max=Math.max(ar.length,br.length);
    let html="";
    for(let i=0;i<max;i++){
      html+=`<tr><td>${E(ar[i]?.name||br[i]?.name||'-')}</td><td>${ar[i]?E(ar[i].level):'-'}</td><td>${br[i]?E(br[i].level):'-'}</td><td>-</td></tr>`;
    }
    passiveTable.innerHTML=html||`<tr><td colspan="4">패시브 데이터 없음</td></tr>`;
  }else{
    passiveTable.innerHTML=names.sort().map(n=>{
      const av=Number(ma[n]?.level||0), bv=Number(mb[n]?.level||0);
      return `<tr><td>${E(n)}</td><td>${av||'-'}</td><td>${bv||'-'}</td><td>${diff(av,bv)}</td></tr>`;
    }).join("")||`<tr><td colspan="4">패시브 데이터 없음</td></tr>`;
  }

  const a20=(a.passives||[]).filter(x=>Number(x.level)>=20).length;
  const b20=(b.passives||[]).filter(x=>Number(x.level)>=20).length;
  passiveKpis.innerHTML=`
    <div class="kpi"><div class="label">20레벨+ 패시브 수</div><div class="value">${a20} / ${b20}</div></div>
    <div class="kpi"><div class="label">평균 패시브 레벨</div><div class="value">${avgLevel(a.passives).toFixed(1)} / ${avgLevel(b.passives).toFixed(1)}</div></div>
    <div class="kpi"><div class="label">평균 차이</div><div class="value">${(avgLevel(a.passives)-avgLevel(b.passives)).toFixed(1)}</div></div>
    <div class="kpi"><div class="label">비교 방식</div><div class="value" style="font-size:17px">${sameJob?"동일명 1:1":"직업별 투자"}</div></div>`;
}

function renderActives(a,b){
  const sameJob=(a.info?.job||"")===(b.info?.job||"");
  activeTitle.textContent=sameJob?`${a.info?.job||''} 액티브 스킬 1:1 비교`:"직업별 핵심 액티브 스킬 비교";
  activeAName.textContent=a.info?.name||"A";
  activeBName.textContent=b.info?.name||"B";

  const ma=levelMap(a.skills), mb=levelMap(b.skills);
  if(sameJob){
    const names=[...new Set([...Object.keys(ma),...Object.keys(mb)])].sort();
    activeTable.innerHTML=names.map(n=>{
      const av=Number(ma[n]?.level||0), bv=Number(mb[n]?.level||0);
      return `<tr><td>${E(n)}</td><td>${av||'-'}</td><td>${bv||'-'}</td><td>${diff(av,bv)}</td></tr>`;
    }).join("")||`<tr><td colspan="4">스킬 데이터 없음</td></tr>`;
  }else{
    const ar=[...(a.skills||[])].sort((x,y)=>Number(y.level)-Number(x.level));
    const br=[...(b.skills||[])].sort((x,y)=>Number(y.level)-Number(x.level));
    const max=Math.max(ar.length,br.length);
    let html="";
    for(let i=0;i<max;i++){
      html+=`<tr><td>${E(ar[i]?.name||br[i]?.name||'-')}</td><td>${ar[i]?E(ar[i].level):'-'}</td><td>${br[i]?E(br[i].level):'-'}</td><td>-</td></tr>`;
    }
    activeTable.innerHTML=html||`<tr><td colspan="4">스킬 데이터 없음</td></tr>`;
  }

  const a20=(a.skills||[]).filter(x=>Number(x.level)>=20).length;
  const b20=(b.skills||[]).filter(x=>Number(x.level)>=20).length;
  activeKpis.innerHTML=`
    <div class="kpi"><div class="label">20레벨 액티브 수</div><div class="value">${a20} / ${b20}</div></div>
    <div class="kpi"><div class="label">평균 액티브 레벨</div><div class="value">${avgLevel(a.skills).toFixed(1)} / ${avgLevel(b.skills).toFixed(1)}</div></div>
    <div class="kpi"><div class="label">평균 차이</div><div class="value">${(avgLevel(a.skills)-avgLevel(b.skills)).toFixed(1)}</div></div>
    <div class="kpi"><div class="label">비교 방식</div><div class="value" style="font-size:17px">${sameJob?"동일명 1:1":"직업별 투자"}</div></div>`;
}

function renderAllSkills(a,b){
  skillHeadA.textContent=`A · ${a.info?.name||'-'} · ${a.info?.job||'-'}`;
  skillHeadB.textContent=`B · ${b.info?.name||'-'} · ${b.info?.job||'-'}`;
  skillsA.innerHTML=(a.skills||[]).map(x=>cardItem(x,"스킬")).join("") + (a.passives||[]).map(x=>cardItem(x,"패시브")).join("");
  skillsB.innerHTML=(b.skills||[]).map(x=>cardItem(x,"스킬")).join("") + (b.passives||[]).map(x=>cardItem(x,"패시브")).join("");
}

function render(d){
  DATA=d;
  charA.innerHTML=profileCard(d.a,'a');
  charB.innerHTML=profileCard(d.b,'b');
  renderGaps(d);
  renderAttackStats(d.a,d.b);
  renderPriorities(d.proAnalysis||{});
  renderEquipment(d.a,d.b);
  renderPassives(d.a,d.b);
  renderActives(d.a,d.b);
  renderAllSkills(d.a,d.b);
  wireCards();
}

async function compare(){
  const q=new URLSearchParams({
    name_a:aName.value.trim(),
    server_a:aServer.value.trim(),
    name_b:bName.value.trim(),
    server_b:bServer.value.trim()
  });

  loadStatus.textContent="캐릭터 상세정보 · 장비 · 마석 · 아르카나 · 스킬 · 패시브 · 랭킹 분석 중...";

  try{
    const r=await fetch(`/api/compare?${q}`);
    const d=await r.json();
    if(!d.ok){
      loadStatus.textContent="한쪽 캐릭터 상세 조회에 실패했습니다.";
      return;
    }
...
async def openchat(msg: str = ""):
    ...
    if body in ("알림켜기", "알림끄기", "알림상태", "테스트"):
        text = "✅ AION2 v48 서버 정상" if body == "테스트" else "📱 알림 설정은 휴대폰 봇에서 처리됩니다."
