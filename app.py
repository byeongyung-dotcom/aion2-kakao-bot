...
async def openchat_alerts(room: str = ""):
    async with _openchat_alert_lock:
        ...
        # !알림켜기/끄기 is evaluated per room when ?room=... is supplied.
        ...
        # Baseline is per room. A newly enabled/newly discovered room never
        # receives old posts; it begins from the newest current post.
        ...
        # ---- Boss/content alerts: configurable lead times, once PER ROOM ----
        now = datetime.now(KST)
        sent = set(str(x) for x in delivery.get("bossSent", []))
        keep = set()

        # Atomic per-room claims. Once an alert key has been handed to a phone,
        # keep the claim for the lifetime of that event/lead window. Do NOT
        # re-issue it every poll even when ACK fails; preventing spam is the
        # priority. Old claims are removed automatically when their event key
        # falls out of `keep` after the event.
        inflight = {}
        now_epoch = time.time()
        for k, claimed_at in (delivery.get("inflight") or {}).items():
            try:
                inflight[str(k)] = float(claimed_at)
            except Exception:
                continue
        ...
        # Agro alone uses the latest maintenance completion as its anchor.
        ...
        for item_type, name, target in targets:
            ...
            # Keep dedupe keys until well after the scheduled event.
            if minutes > -180:
                for lead in leads:
                    keep.add(event_id + f"|{lead}")

            # Exact alert window: fire only during the first 60 seconds after
            # the configured lead time is reached. Never send a missed alert late.
            # Example: 21:00 event + 30m lead => only 20:30:00~20:30:59.
            for lead in leads:
                key = event_id + f"|{lead}"
                if (
                    (lead - 1) < minutes <= lead
                    ...
                    # Claim BEFORE returning the item. Because this whole route
                    # is under _openchat_alert_lock, concurrent pollers for the
                    # same room cannot all receive the same alert.
