...
app = FastAPI(title="AION2 Server v54 LockedApprovedUI")
...
function renderLevelTable(a,b,type){
  ...
  const ra=passive?(a.passives||[]):(a.skills||[]), rb=passive?(b.passives||[]):(b.skills||[]);
...
function renderBottomCards(d){
  ...
  $("skillsA").innerHTML=[...(d.a.skills||[]),...(d.a.passives||[])].map(x=>cardHTML(x,x.category||"스킬")).join("")||emptyCards("스킬",2);
  $("skillsB").innerHTML=[...(d.b.skills||[]),...(d.b.passives||[])].map(x=>cardHTML(x,x.category||"스킬")).join("")||emptyCards("스킬",2);
}
...
async def openchat(msg: str = ""):
    ...
    if body in ("알림켜기", "알림끄기", "알림상태", "테스트"):
        text = "✅ AION2 v54 서버 정상" if body == "테스트" else "📱 알림 설정은 휴대폰 봇에서 처리됩니다."
