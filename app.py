...
app = FastAPI(title="AION2 Server v8 Fast AllServer")
...
def format_character_from_data(info, stones):
    ...
    if stones:
        ...
        lines.append("• 마석 정보 없음")

    # Put the card URL last so Kakao can make a link preview.
    lines += ["", "🖼 프로필", character_card_url(info["name"], info["server"])]
    return "\n".join(lines)
...
def format_character_multiple(nickname, items):
    ...
    for i, item in enumerate(items[:10], 1):
        ...
        lines.append(f"{i}. {info['server']} · {info['job']} · {cp}")
...
async def openchat(msg: str = ""):
    ...
    # Character
    try:
        result = await asyncio.wait_for(character_lookup(body), timeout=6.5)
    except asyncio.TimeoutError:
