...
app = FastAPI(title="AION2 Server v15 OneBossAlert")
...
def format_character_from_data(info, stones):
    ...
    lines = [f"⚔️ {name}"]
    if server and server != "확인 실패":
        lines.append(f"{server} · {job} · {cp_short}")
    else:
        if job and job != "확인 실패":
            lines.append(f"{job} · {cp_short}")
        else:
            lines.append(f"전투력 {cp_short}")

    if server and server != "확인 실패":
        lines += ["", "🖼 프로필 보기", character_card_url(name, server)]
...
def format_character_multiple(nickname, items):
    lines = [f"🔎 {nickname} · 전 서버 검색", ""]
    shown = 0
    for item in items[:10]:
        info = item["info"]
        server = info.get("server") or ""
        job = info.get("job") or ""
        cp = round(info["combatPower"] / 1000) if info.get("combatPower") else "-"
        if not server or server == "확인 실패":
            continue
        lines.append(f"• {server} · {job} · {cp}")
        shown += 1

    if shown:
        lines += ["", f"바로검색  !지켈{nickname}  /  !{nickname}지켈"]
    else:
        lines = [f"🔎 {nickname}", "검색 결과는 있으나 서버 정보를 확인하지 못했습니다."]
    return "\n".join(lines)
...
def format_one_boss(cache, query):
    ...
    matches = [row for row in rows if q in normalize_boss_query(row["name"])]
    if not matches:
        return f"🐲 {query} · 지켈\n\n출현 시간을 찾지 못했습니다."
...
def format_board_latest(command: str, rows):
    if command == "공지":
        rows = [r for r in rows if "점검" in str(r.get("title") or "")]
        if not rows:
            return "🔧 AION2 점검 공지\n\n현재 확인되는 점검 공지가 없습니다."
        row = rows[0]
        return (
...
def _load_openchat_alert_state():
    try:
        ...
        raw = json.loads(
            OPENCHAT_ALERT_STATE_FILE.read_text(encoding="utf-8")
        )
        ...
        # 반드시 이전에 보낸 보스 알림 키를 다시 읽어온다.
        # 이 값이 빠지면 1분 폴링마다 같은 알림이 반복된다.
        if isinstance(raw.get("bossSent"), list):
            state["bossSent"] = [
                str(x) for x in raw["bossSent"]
            ]
...
async def openchat_alerts():
    ...
    async with _openchat_alert_lock:
        ...
        # Boss alerts: fire exactly once around 30 minutes before spawn.
        ...
        for row in rows:
            ...
            if not target_at:
                continue
            mins = (target_at - now_ms) / 60000.0
            key = f"{name}|{target_at}"

            # Retain recent/future keys, discard stale ones.
            if mins > -180:
                keep.add(key)

            if 29 <= mins <= 31.5 and key not in sent:
                target = datetime.fromtimestamp(target_at / 1000, tz=KST)
                items.append({
                    "type": "boss",
                    "boss": name,
        ...
        return {"ok": True, "baseline": first_run, "items": items}
...
async def debug_v14():
    result = {"version": "v14"}
    try:
        chars = await search_characters_all_servers("윤이")
        result["character"] = {"ok": True, "count": len(chars)}
    except Exception as e:
        result["character"] = {"ok": False, "error": str(e)[:200]}
    try:
        cache = await fetch_field_boss_cache()
        result["fieldBoss"] = {"ok": True, "count": len(zikel_boss_entries(cache))}
    except Exception as e:
        result["fieldBoss"] = {"ok": False, "error": str(e)[:200]}
    try:
        rows = await fetch_board_latest("공지", limit=18)
        result["notice"] = {"ok": True, "count": len(rows), "maintenance": len([r for r in rows if "점검" in r["title"]])}
    except Exception as e:
        result["notice"] = {"ok": False, "error": str(e)[:200]}
    return result
