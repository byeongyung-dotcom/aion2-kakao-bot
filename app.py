...
app = FastAPI(title="AION2 Server v14 StablePretty")
...
async def openchat_alerts():
    ...
    async with _openchat_alert_lock:
        ...
        # Boss alerts: fire once when within 25~35 min, resilient to 1-min polling jitter.
        ...
        for row in rows:
            ...
            if 25 <= mins <= 35 and key not in sent:
                target = datetime.fromtimestamp(target_at / 1000, tz=KST)
                items.append({
                    "type": "boss",
                    "boss": name,
                    "time": target.strftime("%H:%M"),
                    "minutes": max(0, int(round(mins))),
                })
                sent.add(key)
                keep.add(key)
