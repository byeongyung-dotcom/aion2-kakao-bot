...
app = FastAPI(title="AION2 Server v50 DeployFinal")
...
<div class="wrap">
  <div class="hero">
    ...
    <div class="sub">미리보기용 UI · 아래 수치는 화면 검토를 위한 샘플값</div>
    <div class="search">
  ...
  <div class="panel section">
    <div class="head" id="passiveTitle">패시브 1:1 비교</div>
    <div class="note">같은 직업은 실제 패시브 이름을 그대로 맞춰서 내 레벨 / 상대 레벨 / 차이로 표시합니다.</div>
  ...
  <div class="panel section">
    <div class="head" id="activeTitle">액티브 스킬 1:1 비교</div>
    <div class="note">같은 직업은 실제 액티브 스킬 이름을 1:1로 맞춰 레벨 차이를 표시합니다.</div>
    <table style="margin-top:10px">
...
const DS=(a,b,suffix="")=>{const d=Number(a||0)-Number(b||0);return `<span class="${d>0?'good':d<0?'bad':''}">${d>0?'+':''}${F(d)}${suffix}</span>`};

function profileCard(x,side){
  const i=x.info||{}, p=(DATA?.proAnalysis||{})[side==="a"?"jobProfileA":"jobProfileB"]||{};
  return `<div class="char">${i.profileImage?`<img class="avatar" src="${E(i.profileImage)}" style="object-fit:cover">`:`<div class="avatar">${E((i.name||'?').slice(0,1))}</div>`}<div><div class="name">${E(i.name||"-")}</div><div class="meta">${E(i.server||"-")} · ${E(i.job||"-")} · Lv.${F(i.level)}</div><div class="cp">${Math.round(Number(i.combatPower||0)/1000)}K</div><div class="badges"><span class="badge">${E(p.directionLabel||"포지션 수동")}</span>${Number(p.frontRatio)>0?`<span class="badge">전방 ${F(p.frontRatio)}%</span>`:""}${Number(p.backRatio)>0?`<span class="badge">후방 ${F(p.backRatio)}%</span>`:""}</div></div></div>`;
}

function cmap(x){const m={};(x.combinedOffense||[]).forEach(v=>m[v.name]=v);return m}
function renderAttack(a,b){
  const A=cmap(a),B=cmap(b),groups=[["공격 기반",["공격력","추가 공격력","PVE 공격력","보스 공격력","공격력 증가율"]],["피해 증폭",["피해 증폭","무기 피해 증폭","PVE 피해 증폭","보스 피해 증폭","전방 피해 증폭","후방 피해 증폭","치명타 피해 증폭"]],["조건·확률",["치명타","강타","명중","관통","공격 속도","시전 속도"]]];
  let h="";
  for(const [g,names] of groups){const rows=names.filter(n=>A[n]||B[n]);if(!rows.length)continue;h+=`<tr class="group"><td colspan="4">${E(g)}</td></tr>`;for(const n of rows){const x=A[n]||{baseValue:0,combinedValue:0,unit:"number"},y=B[n]||{baseValue:0,combinedValue:0,unit:"number"};const xv=x.combinedValue??x.baseValue??0,yv=y.combinedValue??y.baseValue??0,suffix=(x.unit==="percent"||y.unit==="percent")?"%":"";h+=`<tr><td>${E(n)}</td><td>${F(xv)}${suffix}</td><td>${F(yv)}${suffix}</td><td>${DS(xv,yv,suffix)}</td></tr>`}}
  attackStats.innerHTML=h||`<tr><td colspan="4">공격 스탯 데이터 없음</td></tr>`;
}
function renderGap(d){const p=d.proAnalysis||{},a=Number(d.a?.info?.combatPower||0),b=Number(d.b?.info?.combatPower||0),f=(p.priorities||[])[0];gapKpis.innerHTML=`<div class="kpi"><div class="label">전투력 차이</div><div class="value ${a>=b?'good':'bad'}">${a>=b?'+':''}${Math.round((a-b)/1000)}K</div></div><div class="kpi"><div class="label">가장 큰 부족</div><div class="value">${E(f?.name||"-")}</div></div><div class="kpi"><div class="label">격차 회복 기대딜</div><div class="value">${f?`+${Number(f.expectedRecoveryPct||0).toFixed(2)}%`:"-"}</div></div><div class="kpi"><div class="label">분석 직업</div><div class="value" style="font-size:17px">${E(d.a?.info?.job||"-")}</div></div>`}
function renderPriority(p){priorityCards.innerHTML=(p.priorities||[]).slice(0,3).map(r=>`<div class="pcard"><div class="rank">#${E(r.rank)} ${E(r.name)}</div><div class="pct">+${Number(r.expectedRecoveryPct||0).toFixed(2)}%</div><div class="tiny">${E(r.action||"")}</div></div>`).join("")||`<div class="note">검증 가능한 공격 격차가 크지 않음</div>`;const c=p.conditionalChecks||[];priorityNote.textContent=c.length?`조건부 확인: ${c.slice(0,4).map(x=>x.name).join(" · ")} — 단순 숫자 차이만으로 우선순위를 정하지 않습니다.`:"현재 검증 가능한 딜 회복량 기준으로 정렬합니다."}
function openDetail(x){m1.textContent=x.name||"-";m2.textContent=[x.slot,x.grade,x.level!=null?`Lv.${x.level}`:"",x.enhance!=null?`+${x.enhance}`:""].filter(Boolean).join(" · ");let lines=[];if(x.description)lines.push(x.description);(x.magicStones||[]).forEach(s=>lines.push(`💎 ${s.name} ${s.value??""}`));(x.options||[]).forEach(o=>lines.push(`${o.name} ${o.value??""}`));m3.textContent=lines.join("\n")||"추가 상세정보 없음";modal.classList.add("show")}
function hide(){modal.classList.remove("show")}
function card(x,type){const data=encodeURIComponent(JSON.stringify(x));return `<div class="card" data-detail="${data}"><div class="rowcard">${x.icon?`<img class="icon" src="${E(x.icon)}" style="object-fit:cover">`:`<div class="icon">${type==="장비"?"⚔️":type==="아르카나"?"✦":"◆"}</div>`}<div><div class="ctitle">${E(x.name||"-")}</div><div class="tiny">${E(type)} ${x.level!=null?`· Lv.${E(x.level)}`:""} ${x.enhance!=null?`· +${E(x.enhance)}`:""}</div></div></div></div>`}
function wire(){document.querySelectorAll("[data-detail]").forEach(el=>el.onclick=()=>openDetail(JSON.parse(decodeURIComponent(el.dataset.detail))))}
function levelMap(rows){const m={};(rows||[]).forEach(x=>{if(x?.name)m[x.name]=x});return m}
function avg(rows){const a=(rows||[]).map(x=>Number(x.level)).filter(Number.isFinite);return a.length?a.reduce((x,y)=>x+y,0)/a.length:0}
function renderLevel(a,b,type){
  const same=(a.info?.job||"")===(b.info?.job||""),rowsA=type==="passive"?(a.passives||[]):(a.skills||[]),rowsB=type==="passive"?(b.passives||[]):(b.skills||[]),mapA=levelMap(rowsA),mapB=levelMap(rowsB),tbody=type==="passive"?passiveTable:activeTable,title=type==="passive"?passiveTitle:activeTitle,nameA=type==="passive"?passiveAName:activeAName,nameB=type==="passive"?passiveBName:activeBName;
  title.textContent=same?`${a.info?.job||""} ${type==="passive"?"패시브":"액티브 스킬"} 1:1 비교`:`직업별 ${type==="passive"?"패시브":"액티브"} 투자 비교`;nameA.textContent=a.info?.name||"A";nameB.textContent=b.info?.name||"B";
  if(same){const names=[...new Set([...Object.keys(mapA),...Object.keys(mapB)])].sort();tbody.innerHTML=names.map(n=>{const av=Number(mapA[n]?.level||0),bv=Number(mapB[n]?.level||0);return `<tr><td>${E(n)}</td><td>${av||"-"}</td><td>${bv||"-"}</td><td>${DS(av,bv)}</td></tr>`}).join("")||`<tr><td colspan="4">데이터 없음</td></tr>`}else{const aa=[...rowsA].sort((x,y)=>Number(y.level)-Number(x.level)),bb=[...rowsB].sort((x,y)=>Number(y.level)-Number(x.level));let h="",n=Math.max(aa.length,bb.length);for(let i=0;i<n;i++)h+=`<tr><td>${E(aa[i]?.name||bb[i]?.name||"-")}</td><td>${aa[i]?.level??"-"}</td><td>${bb[i]?.level??"-"}</td><td>-</td></tr>`;tbody.innerHTML=h||`<tr><td colspan="4">데이터 없음</td></tr>`}
  const k=type==="passive"?passiveKpis:activeKpis,a20=rowsA.filter(x=>Number(x.level)>=20).length,b20=rowsB.filter(x=>Number(x.level)>=20).length;k.innerHTML=`<div class="kpi"><div class="label">20레벨 이상 수</div><div class="value">${a20} / ${b20}</div></div><div class="kpi"><div class="label">평균 레벨</div><div class="value">${avg(rowsA).toFixed(1)} / ${avg(rowsB).toFixed(1)}</div></div><div class="kpi"><div class="label">평균 차이</div><div class="value ${(avg(rowsA)-avg(rowsB))>=0?'good':'bad'}">${(avg(rowsA)-avg(rowsB)).toFixed(1)}</div></div><div class="kpi"><div class="label">비교 방식</div><div class="value" style="font-size:17px">${same?"동일명 1:1":"직업별 투자"}</div></div>`}
function render(d){
  DATA=d;charA.innerHTML=profileCard(d.a,"a");charB.innerHTML=profileCard(d.b,"b");renderGap(d);renderAttack(d.a,d.b);renderPriority(d.proAnalysis||{});
  eqHeadA.textContent=`A · ${d.a.info?.name||"-"} · ${d.a.info?.job||"-"}`;eqHeadB.textContent=`B · ${d.b.info?.name||"-"} · ${d.b.info?.job||"-"}`;equipA.innerHTML=(d.a.equipment||[]).map(x=>card(x,"장비")).join("")||`<div class="note">장비 데이터 없음</div>`;equipB.innerHTML=(d.b.equipment||[]).map(x=>card(x,"장비")).join("")||`<div class="note">장비 데이터 없음</div>`;
  arcHeadA.textContent=`A · ${d.a.info?.name||"-"}`;arcHeadB.textContent=`B · ${d.b.info?.name||"-"}`;arcA.innerHTML=(d.a.arcana||[]).map(x=>card(x,"아르카나")).join("")||`<div class="note">아르카나 데이터 없음</div>`;arcB.innerHTML=(d.b.arcana||[]).map(x=>card(x,"아르카나")).join("")||`<div class="note">아르카나 데이터 없음</div>`;
  renderLevel(d.a,d.b,"passive");renderLevel(d.a,d.b,"active");skillHeadA.textContent=`A · ${d.a.info?.name||"-"} · ${d.a.info?.job||"-"}`;skillHeadB.textContent=`B · ${d.b.info?.name||"-"} · ${d.b.info?.job||"-"}`;skillsA.innerHTML=(d.a.skills||[]).map(x=>card(x,"스킬")).join("")+(d.a.passives||[]).map(x=>card(x,"패시브")).join("");skillsB.innerHTML=(d.b.skills||[]).map(x=>card(x,"스킬")).join("")+(d.b.passives||[]).map(x=>card(x,"패시브")).join("");wire()}
async function compare(){const q=new URLSearchParams({name_a:aName.value.trim(),server_a:aServer.value.trim(),name_b:bName.value.trim(),server_b:bServer.value.trim()});loadStatus.textContent="상세 데이터 분석 중...";try{const r=await fetch(`/api/compare?${q}`),d=await r.json();if(!d.ok){loadStatus.textContent="한쪽 캐릭터 상세 조회 실패";return}loadStatus.textContent="";render(d)}catch(e){loadStatus.textContent="비교 서버 연결 오류"}}
compareBtn.addEventListener("click",compare);modal.onclick=e=>{if(e.target===modal)hide()};
</script>
</body>
</html>
"""
...
async def openchat(msg: str = ""):
    ...
    if body in ("알림켜기", "알림끄기", "알림상태", "테스트"):
        text = "✅ AION2 v50 서버 정상" if body == "테스트" else "📱 알림 설정은 휴대폰 봇에서 처리됩니다."
