...
app = FastAPI(title="AION2 Server v21 CleanFinal")
...
def profile_info(profile_json, requested_name="", fallback_server="", fallback_row=None):
    ...
    return {
        ...
        "job": profile.get("className") or fallback_row.get("className") or fallback_row.get("job") or "",
        "combatPower": int(combat_power or 0),
        "serverId": int(profile.get("serverId") or row_server_id(fallback_row) or 0),
        "server": profile.get("serverName") or row_server_name(fallback_row) or fallback_server or "",
...
def format_character_from_data(info, stones):
    ...
    combat_power = int(info.get("combatPower") or 0)
...
async def resolve_character(nickname: str, server_name: str | None = None):
    ...
    if any(not item["info"]["server"] or item["info"]["server"] == "" for item in items):
...
async def root():
    return {
        "ok": True,
        "service": "AION2 Server v21 CleanFinal",
...
async def openchat(msg: str = ""):
    ...
    # These must NEVER enter character search.
    # -------------------------------------------------
