...
async def character_card_data(nickname: str, server_name: str):
    # Keep basic character data on the official/live resolver, but restore the
    # equipped magic-stone aggregate for the preview card. The direct command
    # itself still only returns the card URL, so this detail fetch does not add
    # extra text to Kakao.
    resolved = await own_resolve_character(
        nickname,
        server_name,
    ...
    # Fresh detailed profile first so equipped magic stones are current.
    if sid and cid:
        try:
    ...
    # If the live detailed source is temporarily unavailable, reuse only the
    # previously saved full profile for the stone section. Basic info remains
    # the fresh official/live result above.
    if not stones:
        try:
