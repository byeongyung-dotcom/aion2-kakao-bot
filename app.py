    ...
    async with _openchat_alert_lock:
        ...
        for item_type, name, target in targets:
            ...
            for idx, lead in enumerate(leads):
                ...
                key = event_id + f"|{lead}"
                if lower_bound < minutes <= lead and key not in sent:
                    items.append({
                        "type": item_type,
                    ...
                    # IMPORTANT:
                    # Fetching /openchat/alerts does NOT mean Kakao actually
                    # delivered the message. The phone bot ACKs only after
                    # Api.replyRoom succeeds, so a transient phone/network
                    # failure cannot silently consume the alert.
                    keep.add(key)
