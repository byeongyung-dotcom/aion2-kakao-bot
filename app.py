...
app = FastAPI(title="AION2 Server v51 ExactPreviewUI")
...
<div class="wrap">
  <div class="hero">
    ...
    <div class="sub" id="loadStatus">캐릭터명 / 서버 형식으로 입력</div>
    <div class="search">
      <input id="queryA" value="윤이 / 지켈">
      <input id="queryB" value="쵸비 / 지켈">
...
function detail(x){
  m1.textContent=x.name||"-";m2.textContent=[x.slot,x.grade,x.level!=null?`Lv.${x.level}`:"",x.enhance!=null?`+${x.enhance}`:""].filter(Boolean).join(" · ");
  let a=[];if(x.description)a.push(x.description);(x.magicStones||[]).forEach(s=>a.push(`💎 ${s.name} ${s.value??""}`));(x.options||[]).forEach(o=>a.push(`${o.name} ${o.value??""}`));m3.textContent=a.join("\n")||"추가 상세정보 없음";modal.classList.add("show");
}
...
function wire(){document.querySelectorAll("[data-x]").forEach(el=>el.onclick=()=>detail(JSON.parse(decodeURIComponent(el.dataset.x))))}
...
function render(d){
  DATA=d;charA.innerHTML=profileHTML(d.a,"a");charB.innerHTML=profileHTML(d.b,"b");renderGap(d);renderAttack(d.a,d.b);renderPriority(d.proAnalysis||{});
...
async function compare(){
  const A=parseQuery(queryA.value),B=parseQuery(queryB.value);
  ...
  try{
    const r=await fetch(`/api/compare?${q}`),d=await r.json();
    if(!d.ok){loadStatus.textContent="한쪽 캐릭터 상세 조회 실패";return}
    loadStatus.textContent="";render(d);
  }catch(e){loadStatus.textContent="비교 서버 연결 오류"}
}
...
async def openchat(msg: str = ""):
    ...
    if body in ("알림켜기", "알림끄기", "알림상태", "테스트"):
        text = "✅ AION2 v51 서버 정상" if body == "테스트" else "📱 알림 설정은 휴대폰 봇에서 처리됩니다."
