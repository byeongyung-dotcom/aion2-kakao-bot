...
async def alerts_check(secret: str = ""):
    # Legacy Kakao Event delivery is intentionally disabled.
    # MessengerBotR/OpenChat must use /openchat/alerts only, otherwise the same
    # notice can be delivered twice by two independent alert pipelines.
    return {
        "ok": True,
        "disabled": True,
        "message": "legacy alert pipeline disabled; use /openchat/alerts",
    }
...
def _default_openchat_alert_state_path():
    env_path = str(os.getenv("OPENCHAT_ALERT_STATE_FILE") or "").strip()
    if env_path:
        return Path(env_path)
    # Keep alert state beside the character DB. If CHARACTER_DB_PATH points to
    # Render Persistent Disk (/var/data), alert de-duplication survives restarts.
    return Path(CHARACTER_DB_PATH).with_name("aion2_openchat_alert_state.json")

OPENCHAT_ALERT_STATE_FILE = _default_openchat_alert_state_path()
...
def _alert_was_sent(scope, alert_key):
    try:
        conn = _db_connect()
        try:
            _alert_sent_db_init(conn)
            row = conn.execute(
                "SELECT 1 FROM alert_sent WHERE scope=? AND alert_key=? LIMIT 1",
                (str(scope), str(alert_key)),
            ).fetchone()
...
def _mark_alert_sent(scope, alert_key):
    try:
        conn = _db_connect()
        try:
            _alert_sent_db_init(conn)
            conn.execute(
                "INSERT OR IGNORE INTO alert_sent(scope, alert_key, sent_at) VALUES(?,?,?)",
                (str(scope), str(alert_key), datetime.now(KST).isoformat()),
            )
            conn.commit()
        finally:
            conn.close()
...
async def openchat_alerts_legacy_disabled():
    # Legacy phone scripts used this shared endpoint. Keep it silent so old
    # MessengerBotR scripts cannot duplicate or steal alerts from the final client.
    return {
        "ok": True,
        "disabled": True,
        "version": "legacy-disabled",
        "items": [],
    }


@app.get("/openchat/alerts/v2")
    ...
    async with _openchat_alert_lock:
                ...
                for post in reversed(new_rows):
                    ...
                    if board == "공지":
                        ...
                        if "라이브" in title_text or "live" in lowered:
                            ...
                            # Automatic notice alerts are ONLY maintenance/live.
                            continue

                    post_key = f"{board}:{post['id']}"
                    if _alert_was_sent("board", post_key):
                        continue

                    official_link = str(post.get("link") or "").strip()
                    card_link = board_card_url(board, post["id"])
                    items.append({
                        "type": "board",
                        "board": board,
                        "kind": notice_kind,
                        "id": post["id"],
                        "title": post["title"],
                        # Keep both names for older/newer MessengerBotR scripts.
                        "url": official_link or card_link,
                        "link": official_link or card_link,
                        "cardUrl": card_link,
                    })
                    _mark_alert_sent("board", post_key)
        ...
        for name, target in targets:
            ...
            if (
                27 <= minutes <= 33
                and key not in sent
                and not _alert_was_sent("boss", key)
            ):
                items.append({
                    "type": "boss",
                    "boss": name,
...
async def openchat(msg: str = ""):
    ...
    if body in ("설명", "도움", "명령어", "사용법"):
        return PlainTextResponse(
            ...
            "⚔️ 캐릭터 조회\n"
            "!윤이 - 기본 서버 캐릭터 조회\n"
            "!윤이지켈 / !지켈윤이 / !윤이 지켈 - 서버 지정 조회\n\n"
            "⚔️ 캐릭터 비교\n"
            "!비교 - 캐릭터 2명의 마석·스킬·스티그마·패시브·장비 비교 사이트\n\n"
            "🏆 랭킹\n"
            "!랭킹 윤이지켈 - 캐릭터 랭킹 조회\n\n"
            "🐲 필드보스\n"
            "!필보 - 전체 일정 / !아그로 / !카이라 / !나흐마 / !어비스 - 개별 일정\n\n"
            "📢 공식 소식\n"
            "!공지 / !CM / !업데이트 - 최신 공식 게시글 조회\n\n"
            "👥 기타\n"
            "!인원 - 인원표 열기\n\n"
            "🔔 자동알림\n"
            "공지: 점검·라이브 제목만 / CM·업데이트: 새 글 알림\n"
            "필드보스: 출현 30분 전 1회",
