...
def format_character_from_data(info, stones):
    ...
    lines = [
        f"⚔️ {name} · {server}" if server else f"⚔️ {name}",
        f"{job}  |  전투력 {cp_short:,}" if job and cp_short else (job or (f"전투력 {cp_short:,}" if cp_short else "")),
    ]
    ...
    if clean_stones:
        lines.append("")
        lines.append("💎 마석")
        # Compact two-per-line style where possible.
        row = []
        for sname, sval in clean_stones:
            row.append(f"{sname} {sval}")
            if len(row) == 2:
    ...
    if server:
        # Kakao aggressively caches link previews. CP in the query string makes
        # the preview refresh automatically when the character changes.
        url = character_card_url(name, server)
        if cp:
            url += f"?v={cp}"
        lines += ["", url]
...
async def own_resolve_character(nickname, server_name=None):
    """Character lookup: refresh first, DB only as fallback."""
    nickname = str(nickname or "").strip()
    server_name = str(server_name or "").strip() or None

    # Always try the live character source first so !닉네임서버 reflects
    # current combat power/class/profile instead of returning an old DB row.
    try:
    ...
    # Upstream failure only: fall back to the last saved character data.
