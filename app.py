...
def format_character_from_data(info, stones):
    ...
    if server:
        lines.append(character_card_url(name, server))
...
async def _fresh_official_character(nickname, server_name=None):
    """Use NC official search+info without the 120/180s app caches."""
    _drop_character_live_caches(nickname, server_name)

    rows = await official_search_characters(nickname, server_name)
    if not rows:
        return None

    _drop_character_live_caches(nickname, server_name, rows)

    if server_name:
        sid = SERVER_ID_MAP.get(str(server_name).strip())
        if sid:
            rows = [r for r in rows if int(r.get("serverId") or 0) == int(sid)]
    if not rows:
        return None

    # Exact server lookup should normally leave one candidate.
    ...
    for row, info in zip(rows[:12], details):
        if isinstance(info, Exception) or not isinstance(info, dict):
            continue
        if str(info.get("name") or "").casefold() != str(nickname).casefold():
            continue
        if server_name and str(info.get("server") or "") != str(server_name):
            continue
...
async def openchat_alerts(room: str = ""):
    ...
    async with _openchat_alert_lock:
        ...
        # Test alerts stay retryable for 5 minutes.
        kept_tests = []
