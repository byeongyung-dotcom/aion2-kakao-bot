...
app = FastAPI(title="AION2 Server v39 BuiltinCompare")
...
def extract_profile_stats(profile):
    """
    Generic stat extractor. Keeps human-readable stat name/value pairs found
    in common stat containers without assuming one fixed NotMeter schema.
    """
    stats = {}
        ...
        if isinstance(node, dict):
            for k, v in node.items():
                if isinstance(v, (str, int, float)):
                    ...
                    n = _safe_num(raw)
                    if n is not None:
                        stats[name] = {"name": name, "value": n, "raw": raw}
    ...
    # Fallback: inspect nested objects with name + value, but avoid stones/items.
    if not stats:
        for d in _walk_dicts(profile):
            if "magicStoneStat" in d:
                continue
            name = str(d.get("statName") or d.get("name") or "").strip()
            if not name:
                continue
            if "value" not in d:
                continue
            ...
            if n is not None and len(name) <= 40:
                stats.setdefault(
                    name,
                    {"name": name, "value": n, "raw": d.get("value")},
                )
...
async def detailed_character_data(nickname: str, server_name: str):
    """
    Detailed comparison intentionally asks NotMeter for the full profile because
    equipment/magicStoneStat are not stored in the lightweight own-character DB.
    If the full profile is unavailable, basic DB data is still returned.
    """
    ...
    try:
        ...
        if fresh.get("type") == "detail":
            full_profile = fresh.get("profile") or {}
            full_info = fresh.get("info") or basic
            source_ok = bool(full_profile)
            await _save_notmeter_resolved(fresh)
    except Exception:
        pass
...
<title>AION2 상세 캐릭터 비교</title>
<style>
:root{--bg:#09111f;--panel:#111c30;--panel2:#0c1628;--line:#243653;--txt:#eef5ff;--muted:#9db0cc;--a:#75a7ff;--good:#73d6a0;--bad:#ff8d96;--warn:#ffd166}
*{box-sizing:border-box} body{margin:0;background:linear-gradient(180deg,#07101e,#0d1728);color:var(--txt);font-family:Arial,"Noto Sans KR",sans-serif}
.wrap{max-width:1300px;margin:auto;padding:24px 16px 60px}
.hero,.box{background:rgba(17,28,48,.96);border:1px solid var(--line);border-radius:18px;padding:18px}
h1{margin:0 0 6px;font-size:30px}.muted{color:var(--muted)}.grid2{display:grid;grid-template-columns:1fr 1fr;gap:14px}.search{margin-top:18px}
.row{display:grid;grid-template-columns:1fr 150px;gap:8px}input,button{padding:11px;border-radius:10px;border:1px solid var(--line);background:#0b1528;color:var(--txt)}
button{background:var(--a);color:#07101e;font-weight:800;cursor:pointer}.actions{margin-top:10px}.status{font-size:12px;color:var(--muted);margin-top:8px;min-height:16px}
.hidden{display:none}.section{margin-top:16px}.headline{font-size:20px;font-weight:800;margin-bottom:10px}.char-name{font-size:24px;font-weight:900}.cp{font-size:38px;font-weight:900;margin-top:10px}
table{width:100%;border-collapse:collapse;background:var(--panel2);border-radius:12px;overflow:hidden}th,td{padding:10px;border-bottom:1px solid var(--line);font-size:13px}th{color:var(--muted);text-align:left}td:nth-child(n+2),th:nth-child(n+2){text-align:right}.plus{color:var(--good);font-weight:800}.minus{color:var(--bad);font-weight:800}
.equipment-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.equip{background:var(--panel2);border:1px solid var(--line);border-radius:12px;padding:12px;margin-bottom:8px}.slot{font-size:12px;color:var(--muted)}.ename{font-weight:800;margin:4px 0}.stone{display:inline-block;padding:4px 7px;border-radius:8px;background:#1a2a44;margin:3px 3px 0 0;font-size:12px}
.recs{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.rec{background:var(--panel2);border:1px solid var(--line);padding:13px;border-radius:12px}.rec b{display:block;margin-bottom:6px}
@media(max-width:850px){.grid2,.equipment-grid,.recs{grid-template-columns:1fr}.row{grid-template-columns:1fr}}
</style>
</head>
<body>
<div class="wrap">
  <div class="hero">
    <h1>AION2 상세 캐릭터 비교</h1>
    <div class="muted">장비 · 마석 · 기본 스탯을 한 번에 비교하고 부족한 세팅을 분석합니다.</div>
    <div class="grid2 search">
      <div class="box">
        <b>내 캐릭터</b>
        <div class="row" style="margin-top:8px"><input id="na" value="윤이"><input id="sa" value="지켈"></div>
        <div class="status" id="sta"></div>
      </div>
      <div class="box">
        <b>비교 캐릭터</b>
        <div class="row" style="margin-top:8px"><input id="nb" placeholder="닉네임"><input id="sb" placeholder="서버"></div>
        <div class="status" id="stb"></div>
      </div>
    </div>
    <div class="actions"><button onclick="compare()">상세 비교하기</button></div>
  </div>
  ...
  <div id="result" class="hidden">
    ...
    <div class="box section">
      <div class="headline">기본 스탯 비교</div>
      <table><thead><tr><th>항목</th><th id="ha">A</th><th id="hb">B</th><th>차이</th></tr></thead><tbody id="stats"></tbody></table>
    </div>
    ...
    <div class="box section">
      <div class="headline">전체 장비 비교</div>
      <div class="equipment-grid">
        <div><b id="eqha">A</b><div id="eqa" style="margin-top:8px"></div></div>
        <div><b id="eqhb">B</b><div id="eqb" style="margin-top:8px"></div></div>
      </div>
    </div>

    <div class="box section">
      <div class="headline">세팅 분석</div>
      <div class="recs" id="recs"></div>
    </div>
  </div>
</div>
<script>
const API="https://aion2-kakao-bot.onrender.com";
async function checkApi(){
  try{
    const r=await fetch(`${API}/api/compare-health`);
...
async function compare(){
  ...
  if(!ok){ sta.textContent=stb.textContent="서버 연결 실패 - Render v38 배포 확인"; return; }
  const p=new URLSearchParams({name_a:na.value.trim(),server_a:sa.value.trim(),name_b:nb.value.trim(),server_b:sb.value.trim()});
  sta.textContent=stb.textContent="불러오는 중...";
  try{
    const r=await fetch(`${API}/api/compare?${p}`); const d=await r.json();
    if(!d.ok) throw Error("상세 비교 데이터를 불러오지 못했습니다.");
    sta.textContent=d.a.profileAvailable?"상세 프로필 연결됨":"기본정보만";
    stb.textContent=d.b.profileAvailable?"상세 프로필 연결됨":"기본정보만";
    render(d.a,d.b);
  }catch(e){sta.textContent=stb.textContent=e.message}
}
function card(x){
 const i=x.info||{};
 return `<div class="char-name">${E(i.name)}</div><div class="muted">${E(i.server)} · ${E(i.job)} · Lv.${F(i.level)}</div><div class="cp">${Math.round(Number(i.combatPower||0)/1000)}</div><div class="muted">전투력</div><div style="margin-top:8px">${x.profileAvailable?'상세 장비/마석 연결':'상세 프로필 미수신'}</div>`;
}
...
function render(a,b){
 ca.innerHTML=card(a); cb.innerHTML=card(b);
 ha.textContent=eqha.textContent=a.info.name; hb.textContent=eqhb.textContent=b.info.name;

 const ma=mapStats(a), mb=mapStats(b), names=[...new Set([...Object.keys(ma),...Object.keys(mb)])];
 const basics=[["전투력",{value:a.info.combatPower},{value:b.info.combatPower}],["레벨",{value:a.info.level},{value:b.info.level}]];
 stats.innerHTML=basics.concat(names.map(n=>[n,ma[n]||{value:0},mb[n]||{value:0}])).map(r=>`<tr><td>${E(r[0])}</td><td>${F(r[1].value)}</td><td>${F(r[2].value)}</td><td>${D(r[1].value,r[2].value)}</td></tr>`).join("");

 const sa1=mapStones(a), sb1=mapStones(b), sn=[...new Set([...Object.keys(sa1),...Object.keys(sb1)])];
 stones.innerHTML=sn.length?sn.map(n=>{const x=sa1[n]||{count:0,total:0,average:0},y=sb1[n]||{count:0,total:0,average:0};return `<tr><td>${E(n)}</td><td>${x.count} / ${F(x.total)} / ${Number(x.average).toFixed(2)}</td><td>${y.count} / ${F(y.total)} / ${Number(y.average).toFixed(2)}</td><td>${D(x.total,y.total)}</td></tr>`}).join(""):`<tr><td colspan="4">마석 상세 데이터 없음</td></tr>`;

 eqa.innerHTML=equipment(a.equipment||[]); eqb.innerHTML=equipment(b.equipment||[]);
 recommendations(a,b,sa1,sb1);
 result.classList.remove("hidden");
}
function equipment(rows){
 if(!rows.length)return `<div class="muted">장비 데이터 없음</div>`;
 return rows.map(x=>`<div class="equip"><div class="slot">${E(x.slot)}</div><div class="ename">${x.enhance!=null?`+${E(x.enhance)} `:''}${E(x.name)}</div><div class="muted">${E(x.grade||'')} ${x.itemLevel?`· Lv.${E(x.itemLevel)}`:''}</div><div>${(x.magicStones||[]).map(s=>`<span class="stone">${E(s.name)} ${E(s.value)}</span>`).join("")}</div></div>`).join("");
}
function recommendations(a,b,sa,sb){
 const arr=[]; const gap=Number(b.info.combatPower||0)-Number(a.info.combatPower||0);
 arr.push({t:"전투력 차이",b:gap>0?`상대보다 ${F(gap)} 낮음`:`내 캐릭터가 ${F(Math.abs(gap))} 높거나 같음`});
 const diffs=[...new Set([...Object.keys(sa),...Object.keys(sb)])].map(n=>({n,d:(sb[n]?.total||0)-(sa[n]?.total||0)})).sort((x,y)=>y.d-x.d);
 if(diffs.length&&diffs[0].d>0)arr.push({t:"가장 부족한 마석",b:`${diffs[0].n} 총합 약 ${F(diffs[0].d)} 부족`});
 else arr.push({t:"마석",b:"상대보다 크게 부족한 마석 총합 없음"});
 arr.push({t:"다음 우선순위",b:"동일 직업이면 부족한 마석/스탯 순으로 교체하는 것이 기본. 직업별 효율 가중치는 다음 단계에서 추가 가능."});
 recs.innerHTML=arr.map(x=>`<div class="rec"><b>${E(x.t)}</b><div class="muted">${E(x.b)}</div></div>`).join("");
}
</script>
</body></html>"""
...
async def api_compare(
    ...
    a, b = await asyncio.gather(
        detailed_character_data(name_a, server_a),
        detailed_character_data(name_b, server_b),
    )
...
async def openchat(msg: str = ""):
    ...
    if body in ("알림켜기", "알림끄기", "알림상태", "테스트"):
        text = "✅ AION2 v39 서버 정상" if body == "테스트" else "📱 알림 설정은 휴대폰 봇에서 처리됩니다."
