...
async def debug_api_probe_v3(nickname: str = '윤이', server: str = '지켈'):
    ...
    server_id = 2002
    try:
        row = character_db_get(nickname, server_id)
    except Exception:
        row = None
    character_id = None
    if isinstance(row, dict):
        character_id = row.get('characterId') or row.get('character_id') or row.get('id')
    if not character_id:
        try:
            resolved = await own_resolve_character(nickname, server)
            if isinstance(resolved, dict):
                character_id = resolved.get('characterId') or resolved.get('character_id') or resolved.get('id')
        except Exception:
            pass
    if not character_id:
        return {'ok': False, 'error': 'characterId not found'}
