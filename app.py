...
app = FastAPI(title="AION2 Server v20 AllServerName")
...
def format_character_from_data(info, stones):
    ...
    return "\n".join([
        f"⚔️ {info['name']}",
        f"{info['server']} · {info['job']} · {cp_short}",
        "",
        "🖼 프로필 보기",
...
# =========================================================
# Server-name character search
# !윤이시엘 / !시엘윤이 / !윤이지켈 / !지켈윤이
# =========================================================

# 확인된 serverId는 바로 사용.
# 나머지는 search 결과의 serverName으로 찾고,
# 필요한 경우 fast profile 1회로 자동 확인/학습한다.
SERVER_ID_MAP = {
    "시엘": 2001,
    "지켈": 2002,
    "루미엘": 2004,
...
# 서버명을 앞/뒤에 붙여 쓸 수 있도록 인식할 이름들.
# 긴 이름부터 검사해 닉네임과의 오인식을 줄인다.
SERVER_NAMES = (
    "메스람타에다",
    "에레슈키갈",
    "이스라펠",
...
# 실행 중 새로 확인한 serverId 저장
SERVER_ID_CACHE = dict(SERVER_ID_MAP)
...
async def character_lookup_server_fast(nickname: str, server_name: str):
    """
    1) nickname을 전 서버 fast search
    2) serverId 또는 serverName으로 해당 서버 후보만 선택
    3) 검색 응답에 서버명이 없으면 fast profile로 자동 확인
    4) 선택된 캐릭터 상세 profile은 1명만 조회
    """
    nickname = str(nickname or "").strip()
    server_name = str(server_name or "").strip()

    if not nickname or not server_name:
        return None
    ...
    target_id = SERVER_ID_CACHE.get(server_name)
    target_name = server_name.casefold()

    matched = []

    # 가장 빠른 경로: 검색 결과의 serverId / serverName만 확인
    for row in rows:
        sid = row_server_id(row)
        ...
        if sname and sname.casefold() == target_name:
            matched.append(row)

            if sid:
                SERVER_ID_CACHE[server_name] = sid

    # 서버명이 검색 결과에 없는 경우에만 fast profile로 보완.
    # 일반적으로는 여기까지 오지 않아서 속도 손해가 없다.
    if not matched:
        async def inspect_row(row):
            sid = row_server_id(row)
            cid = row_character_id(row)
            ...
            try:
                profile_json = await get_profile(sid, cid, fast=True)
                info = profile_info(
                    profile_json,
                    nickname,
                    row_server_name(row),
                ...
                if str(info.get("server") or "").casefold() == target_name:
                    SERVER_ID_CACHE[server_name] = int(
                        info.get("serverId") or sid
                    )
                    return row
            except Exception:
                return None
        ...
        inspected = await asyncio.gather(
            *[inspect_row(row) for row in rows[:12]]
        )
        matched = [row for row in inspected if row]
...
async def character_lookup_smart(body: str):
    ...
    # 기존 대괄호 방식도 유지: !윤이[지켈], !윤이[시엘]
    nickname, explicit_server = parse_character_query(body)
    if explicit_server:
        return await character_lookup_server_fast(
            nickname,
            explicit_server,
        )
    ...
    # 서버명 없이 !윤이 -> 기존 전 서버 검색
    return await character_lookup(body)
...
def format_one_boss(cache, query):
    ...
    # If the API contains multiple spawn records for the same boss, list times only.
    display_name = matches[0]["name"]
    lines = [f"🐲 {display_name}", ""]

    seen_times = set()
    for row in matches:
        text = boss_time_text(row["targetAt"])
        if text in seen_times:
            continue
        seen_times.add(text)
        lines.append(f"⏰ {text}")
...
KAIRA_HOURS = (0, 4, 8, 12, 16, 20)

def format_kaira_fixed():
    return "\n".join([
        "🐲 감시자 카이라",
        "",
        "⏰ 00:00",
...
def next_kaira_spawn(now=None):
    now = now or datetime.now(KST)

    for hour in KAIRA_HOURS:
        target = now.replace(
            hour=hour,
            minute=0,
            second=0,
...
async def field_boss_lookup(query=None):
    if query and normalize_boss_query(query) == normalize_boss_query("카이라"):
        return format_kaira_fixed()
...
async def root():
    return {
        "ok": True,
        "service": "AION2 Server v20 AllServerName",
...
async def openchat_alerts():
    async with _openchat_alert_lock:
        ...
        # 카이라: fixed every 4 hours (00/04/08/12/16/20)
        kaira_target = next_kaira_spawn(now)
        kaira_minutes = (kaira_target - now).total_seconds() / 60.0
        kaira_key = f"감시자 카이라|{int(kaira_target.timestamp())}"

        if kaira_minutes > -180:
            keep.add(kaira_key)

        if 29 <= kaira_minutes <= 31 and kaira_key not in sent:
            items.append({
                "type": "boss",
                "boss": "감시자 카이라",
                "time": kaira_target.strftime("%H:%M"),
                "key": kaira_key,
            })
            sent.add(kaira_key)
            keep.add(kaira_key)

        # 아그로 / 나흐마: use current boss API
        try:
            boss_cache = await fetch_field_boss_cache()
            rows = zikel_boss_entries(boss_cache)
        except Exception:
            rows = []

        for row in rows:
            name = str(row.get("name") or "")
            if not any(keyword in name for keyword in ("아그로", "나흐마")):
                continue

            target_at = int(row.get("targetAt") or 0)
            if not target_at:
                continue

            minutes = (target_at - now_ms) / 60000.0
            key = f"{name}|{target_at}"

            if minutes > -180:
                keep.add(key)

            if 29 <= minutes <= 31 and key not in sent:
                target = datetime.fromtimestamp(target_at / 1000, tz=KST)
                items.append({
                    "type": "boss",
                    "boss": name,
...
async def openchat(msg: str = ""):
    ...
    if not body:
        return PlainTextResponse(
            "!윤이 / !필보 / !아그로 / !공지 / !CM / !업데이트",
            media_type="text/plain; charset=utf-8",
        )

    # 1. Fixed commands are NEVER sent to character search.
    if body == "필보":
        try:
            result = await asyncio.wait_for(field_boss_lookup(), timeout=5.5)
        except Exception:
            result = "⚠️ 필드보스 조회 실패"
    ...
    if board_command in ("공지", "CM", "업데이트"):
        try:
            result = await asyncio.wait_for(board_lookup(board_command), timeout=5.5)
        except Exception:
            result = f"⚠️ {board_command} 조회 실패"
        return PlainTextResponse(result, media_type="text/plain; charset=utf-8")

    # 2. Local/reserved bot commands are NEVER character searches.
    ...
    if body in ("알림켜기", "알림끄기", "알림상태"):
        return PlainTextResponse(
            "이 명령은 메신저봇R에서 처리합니다.",
            media_type="text/plain; charset=utf-8",
        )

    # 3. Boss commands are NEVER sent to character search.
    if body in ("아그로", "카이라", "나흐마"):
        try:
            result = await asyncio.wait_for(field_boss_lookup(body), timeout=4.5)
        except Exception:
            result = "⚠️ 필드보스 조회 실패"
        return PlainTextResponse(result, media_type="text/plain; charset=utf-8")
    ...
    # 4. Everything else = character.
    try:
        result = await asyncio.wait_for(character_lookup_smart(body), timeout=7.5)
    except asyncio.TimeoutError:
        result = "⚠️ 캐릭터 조회 지연. 다시 입력해 주세요."
    except Exception:
        result = "⚠️ 캐릭터 조회 실패"
