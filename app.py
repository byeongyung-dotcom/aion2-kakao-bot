            ...
            if isinstance(src, dict) and src.get("ok"):
                ...
                for item in src.get("items") or []:
                    ...
                    for ms in item.get("magicStoneStat") or []:
                        ...
                        if not name:
                            continue
                        raw = str(ms.get("value") or "").strip()
                        clean = raw.replace("+", "").replace(",", "").replace("%", "")
                        try:
                            value = float(clean)
                        except Exception:
                            continue
                        is_percent = ("%" in raw) or (stat_id in pct_ids)
                        if stat_id in pct_ids and "%" not in raw:
                            value /= 100.0
                        g = groups.setdefault(
                            name,
                            {"total": 0.0, "percent": is_percent},
                        )
                        g["total"] += value
                        g["percent"] = bool(g["percent"] or is_percent)

                for name, g in groups.items():
                    value = float(g.get("total") or 0.0)
                    if g.get("percent"):
                        stones.append((name, f"+{pretty_number(value)}%"))
                    else:
                        stones.append((name, f"+{pretty_number(value)}"))
        except Exception:
            stones = []
