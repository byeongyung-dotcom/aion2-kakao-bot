...
app = FastAPI(title="AION2 Server v19 KairaFast")
...
async def character_lookup_server_fast(nickname: str, server_name: str):
    """
    Fast direct lookup for a known server.
    Search once, filter by serverId/serverName, then fetch ONLY the chosen profile.
    """
    ...
    target_id = server_ids.get(server_name)
    ...
    # Highest combat power first if exact nickname exists more than once on same server.
    matched.sort(
        key=lambda r: int(r.get("combatPower") or 0),
        reverse=True,
    )
...
async def character_lookup_smart(body: str):
    ...
    # !지켈윤이
    if body.startswith("지켈") and len(body) > 2:
        nickname = body[2:].strip()
        if nickname:
            return await character_lookup_server_fast(nickname, "지켈")

    # !윤이지켈
    if body.endswith("지켈") and len(body) > 2:
        nickname = body[:-2].strip()
        if nickname:
            return await character_lookup_server_fast(nickname, "지켈")
...
async def root():
    return {
        "ok": True,
        "service": "AION2 Server v19 KairaFast",
