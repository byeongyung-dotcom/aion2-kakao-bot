...
def bot_help_text() -> str:
    return (
        ...
        "⚔️ 캐릭터 비교\n"
        "!비교  → 비교 사이트 열기\n"
        "서버/캐릭터 2명을 입력하면 마석·액티브·스티그마·패시브·장비 옵션을 비교\n\n"
        "🐲 필드보스\n"
        "!필보 / !아그로 / !카이라 / !나흐마 / !어비스\n"
        "보스 이름을 직접 입력해도 조회 가능\n\n"
        "📢 공식 소식\n"
        "!공지  → 최신 점검 공지\n"
        "!CM  → 최신 CM 글\n"
        "!업데이트  → 최신 업데이트\n"
        "자동알림: 공지는 점검·라이브만 / CM·업데이트는 새 글 + 공식 링크\n\n"
        "👥 기타\n"
        "!인원  → 인원표\n"
        "!도움 / !설명 / !명령어 / !사용법"
    )
...
def _default_openchat_alert_state():
    return {
        ...
        # Persistent de-duplication for board notifications.  This is separate
        # from lastSeen so repeated polling/concurrent retries cannot emit the
        # same article twice.
        "boardSent": [],
        "bossSent": [],
    }
...
def _load_openchat_alert_state():
    try:
        ...
        if isinstance(raw.get("boardSent"), list):
            state["boardSent"] = [str(x) for x in raw["boardSent"]][-1000:]
...
def _openchat_should_alert_board(board_name, post):
    """Current board alert policy used by the phone/OpenChat bot.

    공지: only maintenance/live titles.
    CM/업데이트: every new article.
    """
    if board_name == "공지":
        title = str((post or {}).get("title") or "")
        lowered = title.casefold()
        return ("점검" in title) or ("라이브" in title) or ("live" in lowered)
    return board_name in ("CM", "업데이트")


def _openchat_board_message(board_name, post):
    title = str((post or {}).get("title") or "").strip()
    link = str((post or {}).get("link") or "").strip()
    if board_name == "공지":
        low = title.casefold()
        header = "🔴 AION2 라이브 공지" if ("라이브" in title or "live" in low) else "🔧 AION2 점검 공지"
    elif board_name == "CM":
        header = "📢 AION2 CM"
    else:
        header = "🆕 AION2 업데이트"
    return "\n".join(x for x in (header, "", title, "", "🔗 바로 보기", link) if x != "")
        ...
        if first_run:
            for board, rows in latest_by_board.items():
                if rows:
                    ...
                    # Baseline all currently visible posts so a restart cannot
                    # re-send an old article as a new alert.
                    for row in rows:
                        board_sent.add(f"{board}:{row['id']}")
            ...
            for board, rows in latest_by_board.items():
                ...
                # Always advance the marker, including notices filtered out by
                # policy, so general notices are never reconsidered next poll.
                state["lastSeen"][board] = rows[0]["id"]
                ...
                for post in reversed(new_rows):
                    ...
                    if sent_key in board_sent:
                        continue
                    # 공지는 점검/라이브만. CM/업데이트는 새 글 전체.
                    if not _openchat_should_alert_board(board, post):
                    ...
                    notice_kind = None
                    if board == "공지":
                        notice_kind = "live" if ("라이브" in title_text or "live" in title_text.casefold()) else "maintenance"

                    link = str(post.get("link") or "")
                    message = _openchat_board_message(board, post)
                    items.append({
                        "type": "board",
                        "board": board,
                        "kind": notice_kind,
                        ...
                        "link": link,
                        "url": link,
                        "cardUrl": board_card_url(board, post["id"]),
                        # Phone bot can reply with this field directly; it
                        # already contains title + official link.
                        "message": message,
                        "text": message,
                    })
                    board_sent.add(sent_key)

        state["boardSent"] = list(board_sent)[-1000:]
...
async def openchat(msg: str = ""):
    ...
    if not body:
        return PlainTextResponse("사용 가능한 명령어는 !도움 을 입력해 주세요.", media_type="text/plain; charset=utf-8")

    # 봇 전용 명령은 반드시 여기서 끝난다. 캐릭터 검색으로 fall-through 금지.
    if body in ("설명", "도움", "명령어", "사용법"):
        return PlainTextResponse(bot_help_text(), media_type="text/plain; charset=utf-8")
...
async def kakao_skill(request: Request):
    try:
        ...
        if not body:
            return JSONResponse(kakao_text("사용 가능한 명령어는 !도움 을 입력해 주세요."))

        if body in ("설명", "도움", "명령어", "사용법"):
            return JSONResponse(kakao_text(bot_help_text()))

        if body == "비교":
            return JSONResponse(kakao_text(
                "⚔️ AION2 캐릭터 비교\n\n"
                "서버와 캐릭터 2명을 입력해 마석·스킬·스티그마·패시브·장비 옵션을 비교합니다.\n\n"
                "https://aion2-kakao-bot.onrender.com/compare"
            ))
