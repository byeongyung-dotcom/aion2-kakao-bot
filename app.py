...
async def openchat_alerts(room: str = ""):
    async with _openchat_alert_lock:
        ...
        for item_type, name, target in targets:
            ...
            # Robust delivery windows: do not depend on hitting a narrow +/-3 minute slot.
            # For [30, 10], the 30m alert may fire from 30:00 down to >10:00,
            # and the 10m alert from 10:00 down to >0:00. This prevents missed
            # alerts when Render wakes late or one phone poll is delayed.
            for idx, lead in enumerate(leads):
                lower_bound = leads[idx + 1] if idx + 1 < len(leads) else 0
                key = event_id + f"|{lead}"
                if (
                    lower_bound < minutes <= lead
                    and key not in sent
