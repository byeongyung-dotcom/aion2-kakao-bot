...
# =========================================================
# Party composition image - Google Sheet (gid 234073660)
# Commands: !무스펠 / !성역3
# =========================================================

PARTY_SHEET_ID = "1TDkZojKWuHNfu5cl1lpuqVZvTKF6W9-c9WLjga8ihIc"
PARTY_SHEET_GID = "234073660"
PARTY_SECTION_NAMES = ("무스펠", "성역3")
...
def _party_col_name(index_zero_based: int) -> str:
...
async def _party_sheet_rows():
    cache_key = "party-sheet-csv-v1"
    cached = cache_get(cache_key, 30)
    if cached is not None:
        return cached

    client = await get_http_client()
    url = f"https://docs.google.com/spreadsheets/d/{PARTY_SHEET_ID}/export"
    res = await client.get(
        url,
        params={"format": "csv", "gid": PARTY_SHEET_GID},
        headers={
    ...
    res.raise_for_status()
    rows = [list(r) for r in csv.reader(io.StringIO(res.text or ""))]
    cache_set(cache_key, rows)
    return rows


def _party_find_range(rows, section_name: str):
    target = _party_normalize(section_name)
    section_targets = {_party_normalize(x) for x in PARTY_SECTION_NAMES}

    start = None
    for r_idx, row in enumerate(rows):
        cells = [_party_normalize(x) for x in row]
        if any(target == cell or target in cell for cell in cells if cell):
            start = r_idx
            break
    ...
    # End at the next known section title. This keeps 1파티 + 2파티 together.
    end = len(rows) - 1
    for r_idx in range(start + 1, len(rows)):
        cells = [_party_normalize(x) for x in rows[r_idx]]
        found_other = False
        for cell in cells:
            if not cell:
                continue
            for sec in section_targets:
                if sec != target and (cell == sec or sec in cell):
                    found_other = True
                    break
            if found_other:
                break
    ...
    # Trim trailing completely empty rows, but preserve internal blank rows.
    while end > start and not any(str(x or "").strip() for x in rows[end]):
        end -= 1

    # Find only the columns actually used inside this section.
    used_cols = []
    for r_idx in range(start, end + 1):
        for c_idx, value in enumerate(rows[r_idx]):
            if str(value or "").strip():
                used_cols.append(c_idx)
    ...
    left = min(used_cols)
    right = max(used_cols)
    return f"{_party_col_name(left)}{start + 1}:{_party_col_name(right)}{end + 1}"


def _party_extract_block(rows, section_name: str):
    found = _party_find_range(rows, section_name)
    if not found:
        return []

    m = re.fullmatch(r"([A-Z]+)(\d+):([A-Z]+)(\d+)", found)
    if not m:
        return []
    ...
    left = col_to_idx(m.group(1))
    top = int(m.group(2)) - 1
    right = col_to_idx(m.group(3))
    bottom = int(m.group(4)) - 1

    block = []
    for r in range(top, min(bottom + 1, len(rows))):
        src = list(rows[r])
        if len(src) <= right:
            src += [""] * (right + 1 - len(src))
...
def _party_svg(display_name: str, block):
    if not block:
        block = [["데이터를 찾지 못했습니다."]]

    cols = max(len(r) for r in block)
    rows = [r + [""] * (cols - len(r)) for r in block]

    # Cell widths are estimated from Korean/ASCII character counts.
    col_widths = []
    for c in range(cols):
        max_units = 6
        for row in rows:
        ...
        col_widths.append(max(110, min(360, 26 + max_units * 11)))
    ...
    out = [
        ...
        f'<text x="{margin}" y="42" font-family="Noto Sans KR,Apple SD Gothic Neo,sans-serif" font-size="30" font-weight="700" fill="#111827">{esc(display_name)} 파티편성</text>',
    ]

    y0 = title_h
    for r_idx, row in enumerate(rows):
        row_text = " ".join(row)
        headerish = (r_idx == 0) or ("1파티" in row_text) or ("2파티" in row_text)
        fill = "#eef4ff" if r_idx == 0 else ("#f3f4f6" if headerish else ("#ffffff" if r_idx % 2 == 0 else "#fafafa"))
        weight = "700" if headerish else "500"
        x = margin
        for c, cell in enumerate(row):
            w = col_widths[c]
            y = y0 + r_idx * row_h
            out.append(f'<rect x="{x}" y="{y}" width="{w}" height="{row_h}" fill="{fill}" stroke="#d1d5db" stroke-width="1"/>')
            text = str(cell or "")
            # Keep long names from spilling into adjacent cells.
            max_chars = max(4, int((w - 24) / 12))
            if len(text) > max_chars:
                text = text[:max_chars - 1] + "…"
            out.append(f'<text x="{x + 12}" y="{y + 32}" font-family="Noto Sans KR,Apple SD Gothic Neo,sans-serif" font-size="21" font-weight="{weight}" fill="#111827">{esc(text)}</text>')
            x += w
...
async def party_image(section_name: str):
    name = _party_normalize(section_name)
    allowed = {_party_normalize(x): x for x in PARTY_SECTION_NAMES}
    if name not in allowed:
        return PlainTextResponse("지원하지 않는 파티편성입니다.", status_code=404)

    display_name = allowed[name]
    try:
        rows = await _party_sheet_rows()
        block = _party_extract_block(rows, display_name)
        if not block:
            return PlainTextResponse(f"{display_name} 구간을 찾지 못했습니다.", status_code=404)

        svg = _party_svg(display_name, block)
        return Response(
        ...
        return PlainTextResponse(
            f"파티편성 이미지 생성 실패: {type(e).__name__}: {str(e)[:180]}",
            status_code=500,
        )
...
async def openchat(msg: str = "", room: str = ""):
    ...
    if body in ("무스펠", "성역3"):
        url = (
            "https://aion2-kakao-bot.onrender.com/party-card/"
            + quote(body, safe="")
        )
        return PlainTextResponse(
            f"👥 {body} 파티편성\n{url}",
