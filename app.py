...
async def own_resolve_character(nickname, server_name=None):
    """Character lookup: live search first, DB only if the live source fails."""
    nickname = str(nickname or "").strip()
    server_name = str(server_name or "").strip() or None

    # Direct character commands must not reuse the 3-minute character-search
    # cache. Remove only this character's search cache before the live lookup.
    ...
    try:
        ...
        if fresh and fresh.get("type") != "none":
            # The search result is the freshest source for combat power.
            # A detailed profile response can lag behind it, so prefer the
            # fresh row for the three fields shown by !닉네임서버.
            if fresh.get("type") == "detail":
                row = fresh.get("row") or {}
                info = dict(fresh.get("info") or {})

                row_cp = row.get("combatPower")
                if row_cp is None:
                    row_cp = row.get("power")
                try:
                    row_cp = int(row_cp or 0)
                except Exception:
                    row_cp = 0
                if row_cp:
                    info["combatPower"] = row_cp
    ...
    # Live source failure only: fall back to the last saved character data.
