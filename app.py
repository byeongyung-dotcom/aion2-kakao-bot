...
<div class="wrap">
  ...
  <div class="panel section">
    ...
    <div class="kpis" id="gapKpis">
      <div class="kpi"><div class="label">전투력 차이</div><div class="value">—</div></div>
      <div class="kpi"><div class="label">가장 큰 부족</div><div class="value">—</div></div>
      <div class="kpi"><div class="label">2순위 부족</div><div class="value">—</div></div>
    </div>
  </div>
...
function renderEquipmentOwner(character, peerCharacter, headId, listId){
  ...
  $(listId).innerHTML=rows.map((r,rowIndex)=>{
    ...
    const exceed=Number(r?.exceedLevel||0)>0?` · 초월 ${Number(r.exceedLevel)}`:'';
    ...
    const subCells=Array.from({length:8},(_,i)=>{
      ...
      const different=!peer || !pm.has(key) || pm.get(key)!==value;
    ...
    return `<div class="equip-detail-card"><div class="equip-detail-left"><div class="equip-big-icon">${icon}</div><div><div class="equip-item-slot">${E(r?.slot||'장비')}</div><div class="equip-item-title">${E(enh+(r?.name||'-'))}${E(exceed)}</div><div class="equip-mainstats">${mainCells}</div><div class="equip-source">${source?`출처 · ${E(source)}`:'출처 · —'}</div></div></div><div class="equip-right">${subCells}</div></div>`;
  }).join('');
}
function renderEquipment(a,b){renderEquipmentOwner(a,b,'equipHeadA','equipListA');renderEquipmentOwner(b,a,'equipHeadB','equipListB');}
