...
function renderEquipmentOwner(character, peerCharacter, headId, listId){
  ...
  $(listId).innerHTML=rows.map((r,rowIndex)=>{
    ...
    const stones=Array.isArray(r?.magicStoneStat)?r.magicStoneStat:[];
    ...
    const right=[
      ...
      stones.length?`<div class="equip-extra-group"><div class="equip-extra-title">마석</div>${renderOptionRows(stones,peer?.magicStoneStat||[],'equip-stone')}</div>`:'',
