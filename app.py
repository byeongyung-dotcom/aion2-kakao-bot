...
app = FastAPI(title="AION2 Server v18 DirectZikel")
...
async def root():
    return {
        "ok": True,
        "service": "AION2 Server v18 DirectZikel",
...
async def openchat_alerts():
    async with _openchat_alert_lock:
        ...
        # ---- Boss alerts: 아그로 / 카이라 / 나흐마 only ----
        target_keywords = ("아그로", "카이라", "나흐마")
        now = datetime.now(KST)
        ...
        for row in rows:
            ...
            # Keep recent/future sent keys; old records are discarded.
            if minutes > -180:
                keep.add(key)

            # The poll runs every minute. One 2-minute window is enough.
...
async def openchat(msg: str = ""):
    ...
    # 2. Known boss commands are NEVER sent to character search.
    ...
    if known_boss_match:
        try:
            result = await asyncio.wait_for(field_boss_lookup(body), timeout=5.5)
        except Exception:
            result = "⚠️ 필드보스 조회 실패"
        return PlainTextResponse(result, media_type="text/plain; charset=utf-8")
