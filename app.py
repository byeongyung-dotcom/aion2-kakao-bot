...
.equip-detail-compare{display:grid;grid-template-columns:1fr 1fr;gap:14px}.equip-detail-owner{border:1px solid var(--line);border-radius:14px;padding:12px;background:#0b1524}.equip-detail-owner.a{border-color:#365f97}.equip-detail-owner.b{border-color:#674886}.equip-detail-head{font-size:16px;font-weight:900;margin-bottom:10px}.equip-work-summary{display:grid;gap:6px;min-height:86px;margin:0 0 10px;padding:8px 9px;border:1px solid #263851;border-radius:10px;background:#0d1828}.equip-work-row{display:grid;grid-template-columns:72px minmax(0,1fr);gap:8px;align-items:start;font-size:11px}.equip-work-label{font-weight:900;color:#9fc3ff;padding-top:2px}.equip-work-values{display:flex;gap:5px;flex-wrap:wrap;min-height:22px}.equip-work-chip{padding:3px 6px;border-radius:6px;border:1px solid #2b405c;background:#111f34;color:#dbe7f7;white-space:nowrap}.equip-section-label{margin-top:7px;margin-bottom:4px;font-size:10px;font-weight:900;color:#9fc3ff}.equip-exceed.diff{color:#ff6b7a;font-weight:900}.equip-exceed{color:#ffb36b}.equip-detail-list{display:grid;gap:10px}.equip-detail-card{display:grid;grid-template-columns:minmax(0,1.2fr) minmax(190px,.8fr);gap:14px;border:1px solid #263851;border-radius:12px;background:#0e1a2b;padding:10px;min-height:210px;height:210px;overflow:hidden}.equip-detail-left{display:grid;grid-template-columns:52px minmax(0,1fr);gap:10px;min-width:0}.equip-big-icon{width:52px;height:52px;border-radius:8px;border:1px solid #31435f;background:#142033;overflow:hidden}.equip-big-icon img{width:100%;height:100%;object-fit:cover}.equip-item-title{font-size:13px;font-weight:900;color:#ff7a2f;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.equip-item-slot{font-size:10px;color:#7fddff;margin-bottom:3px}.equip-mainstats{display:grid;grid-template-columns:1fr 1fr;grid-template-rows:repeat(3,24px);gap:3px 12px;margin-top:8px;min-height:78px}.equip-mainstat{font-size:11px;color:#dbe7f7;min-height:24px;display:flex;align-items:center;overflow:hidden}.equip-source{height:22px;margin-top:6px;font-size:10px;color:var(--muted);display:flex;align-items:center;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.equip-right{border-left:1px solid #2a5267;padding-left:10px;display:grid;grid-template-rows:repeat(8,20px);gap:3px;align-content:start;min-width:0}.equip-substat{display:flex;justify-content:space-between;align-items:center;gap:8px;border:1px solid #20475b;border-radius:6px;padding:2px 7px;font-size:11px;background:#09202b;min-height:20px;overflow:hidden}.equip-substat span{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.equip-substat b{color:#fff;white-space:nowrap}.equip-substat.diff{border-color:#7f2d38;background:#2a1018;color:#ff9aa6}.equip-substat.diff b{color:#ff6b7a}.equip-substat.empty{visibility:hidden}.equip-empty{color:var(--muted);padding:10px}.stone-diff-wrap th,.stone-diff-wrap td{font-size:12px}
...
<div class="panel section">
  ...
  <div class="panel section">
    ...
    <div class="equip-detail-compare" style="margin-top:10px">
      <div class="equip-detail-owner a"><div class="equip-detail-head" id="equipHeadA">A 장비</div><div class="equip-work-summary" id="equipWorkA"></div><div class="equip-detail-list" id="equipListA"></div></div>
      <div class="equip-detail-owner b"><div class="equip-detail-head" id="equipHeadB">B 장비</div><div class="equip-work-summary" id="equipWorkB"></div><div class="equip-detail-list" id="equipListB"></div></div>
    </div>
  </div>
...
function compactWorkRows(rows){
  return filterLevelRows(rows||[]).map(x=>`${E(x.name||'-')} Lv.${Number(x.level||0)}`);
}
function renderEquipmentWork(character, targetId){
  const el=$(targetId); if(!el)return;
  const skills=[...(character?.skills||[]),...(character?.stigma||[])];
  const passives=character?.passives||[];
  const skillRows=compactWorkRows(skills), passiveRows=compactWorkRows(passives);
  const chips=arr=>arr.length?arr.map(x=>`<span class="equip-work-chip">${x}</span>`).join(''):'<span class="equip-work-chip">데이터 없음</span>';
  el.innerHTML=`<div class="equip-work-row"><div class="equip-work-label">스킬 작업</div><div class="equip-work-values">${chips(skillRows)}</div></div><div class="equip-work-row"><div class="equip-work-label">패시브 작업</div><div class="equip-work-values">${chips(passiveRows)}</div></div>`;
}
function renderEquipmentOwner(character, peerCharacter, headId, workId, listId){
  $(headId).textContent=`${character?.info?.name||'캐릭터'} 장비`;
  renderEquipmentWork(character,workId);
  ...
  $(listId).innerHTML=rows.map((r,rowIndex)=>{
    ...
    const main=(r?.mainStats||[]).slice(0,6);
    const subs=(r?.subStats||[]).slice(0,8);
    const pm=tuneMap(peer);
    const mainCells=Array.from({length:6},(_,i)=>main[i]?`<div class="equip-mainstat">${E(statText(main[i]))}</div>`:'<div class="equip-mainstat"></div>').join('');
    const subCells=Array.from({length:8},(_,i)=>{
      const x=subs[i];
      if(!x)return '<div class="equip-substat empty"><span>—</span><b>—</b></div>';
      const key=String(x?.name||x?.id||'-').trim();
      const value=tuneValue(x);
      // User rule: same option name with a different value is NOT highlighted.
      // Red only when the peer item does not have this tuning option at all.
      const different=!peer || !pm.has(key);
      return `<div class="equip-substat${different?' diff':''}"><span>${E(key)}</span><b>${E(value||'-')}</b></div>`;
    }).join('');
    const source=(Array.isArray(r?.sources)?r.sources:[]).filter(Boolean).join(' · ');
    return `<div class="equip-detail-card"><div class="equip-detail-left"><div class="equip-big-icon">${icon}</div><div><div class="equip-item-slot">${E(r?.slot||'장비')}</div><div class="equip-item-title">${E(enh+(r?.name||'-'))}${exceed}</div><div class="equip-section-label">기본 스탯</div><div class="equip-mainstats">${mainCells}</div><div class="equip-source">${source?`출처 · ${E(source)}`:'출처 · —'}</div></div></div><div class="equip-right"><div class="equip-section-label" style="grid-row:auto;height:14px;margin:0 0 2px">조율 옵션</div>${subCells}</div></div>`;
  }).join('');
}
function renderEquipment(a,b){renderEquipmentOwner(a,b,'equipHeadA','equipWorkA','equipListA');renderEquipmentOwner(b,a,'equipHeadB','equipWorkB','equipListB');}
