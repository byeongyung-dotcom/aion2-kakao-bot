...
def _clear_schedule_delivery_keys(name):
    """Drop stale sent keys for one schedule after its alert settings change."""
    canonical = {
        "agro": "정령왕 아그로",
        "kaira": "감시자 카이라",
        "nahma": "수호신장 나흐마",
    ...
    try:
        ...
        for delivery in (state.get("deliveries") or {}).values():
            ...
            new = [k for k in old if not str(k).startswith(prefix)]
            if new != old:
                delivery["sentKeys"] = new
                changed = True
            leases = delivery.get("leases") if isinstance(delivery.get("leases"), dict) else {}
            new_leases = {k: v for k, v in leases.items() if not str(k).startswith(prefix)}
...
async def openchat_alerts(room: str = ""):
    ...
    async with _openchat_alert_lock:
        ...
        # Short server lease: prevent simultaneous duplicate pollers, but do NOT
        # permanently consume an alert merely because it was fetched. If Kakao
        # delivery fails, the same key becomes available again after the lease.
        # The phone keeps its own successful-send dedupe; /ack can permanently
        # confirm a key when a client supports it.
...
async def character_card_stones_latest(name: str = "", server: str = ""):
    """Fast mounted-stone refresh for an opened character card."""
    ...
    try:
        ...
        if saved_profile:
            saved_stones = aggregate_magic_stones(saved_profile)
    except Exception:
        pass
    ...
    try:
        ...
        sid = int(SERVER_ID_MAP.get(server_name) or 0)
        cid = str((row or {}).get("characterId") or (row or {}).get("character_id") or "").strip()

        if not cid:
            resolved = await asyncio.wait_for(
                own_resolve_character(nickname, server_name),
                timeout=2.2,
            )
            if isinstance(resolved, dict) and resolved.get("type") == "detail":
                r = resolved.get("row") or {}
                cid = row_character_id(r)
                sid = row_server_id(r) or sid

        if cid and sid:
            # Fast profile first. This avoids waiting up to five seconds every
            # time the card is opened while still bypassing the saved snapshot.
            fresh_profile = await asyncio.wait_for(
                get_profile(sid, cid, fast=True),
                timeout=2.6,
            )
            fresh_stones = aggregate_magic_stones(fresh_profile) if fresh_profile else []
            if fresh_profile and fresh_stones:
                try:
                    await character_db_save_full_profile(
                        nickname, server_name, fresh_profile, cid
                    )
    ...
    # Never hold the card open waiting for the slow profile path. The saved
    # snapshot is already rendered in the HTML and is returned immediately here.
    return {
        "ok": bool(saved_stones),
        "fresh": False,
        "stones": [
            {"name": n, "value": v}
            for n, v in saved_stones
        ],
    }
