...
async def openchat_alerts(room: str = ""):
    async with _openchat_alert_lock:
        ...
        # Atomic per-room claim with a short retry lease.
        # One poller gets the item at a time. If the phone never ACKs (temporary
        # app/network failure), the claim expires and a later poll can retry.
        inflight = {}
        now_epoch = time.time()
        ...
        for item_type, name, target in targets:
            ...
            # Reliable alert window: allow up to 3 minutes after the configured
            # lead time so Android background throttling or network delay does not
            # miss the alert. It is still never sent much later as a catch-up.
            # Example: 21:00 event + 30m lead => 20:30:00~20:32:59 only.
            for lead in leads:
                key = event_id + f"|{lead}"
                if (
                    (lead - 3) < minutes <= lead
