...
async def official_compare_character_data(nickname: str, server_name: str):
    ...
    attack_flat = sv('main.value','WeaponFixingDamage') + sv('main.extra','WeaponFixingDamage') + sv('sub','WeaponFixingDamage') + sv('magicStone','WeaponFixingDamage')
    ...
    additional_accuracy = sv('main.value','Accuracy') + sv('main.extra','Accuracy') + sv('magicStone','Accuracy')
    critical_flat = sv('main.value','Critical') + sv('main.extra','Critical') + sv('sub','Critical') + sv('magicStone','Critical')
    ...
    weapon_amp = sv('main.value','AmplifyWeaponDamage') + sv('main.extra','AmplifyWeaponDamage') + sv('sub','AmplifyWeaponDamage') + pct_stone('AmplifyWeaponDamage')
    damage_amp = sv('main.value','AmplifyAllDamage') + sv('main.extra','AmplifyAllDamage') + sv('sub','AmplifyAllDamage') + pct_stone('AmplifyAllDamage')
    pve_amp = sv('main.value','PvEAmplifyDamage') + sv('main.extra','PvEAmplifyDamage') + sv('sub','PvEAmplifyDamage') + pct_stone('PvEAmplifyDamage')
    boss_amp = sv('main.value','AmplifyBossDamage') + sv('main.extra','AmplifyBossDamage') + sv('sub','AmplifyBossDamage') + pct_stone('AmplifyBossDamage')
    crit_dmg_amp = sv('main.value','AmplifyCriticalDamage') + sv('main.extra','AmplifyCriticalDamage') + sv('sub','AmplifyCriticalDamage') + pct_stone('AmplifyCriticalDamage')
    back_amp = sv('main.value','AmplifyBackAttack') + sv('main.extra','AmplifyBackAttack') + sv('sub','AmplifyBackAttack') + pct_stone('AmplifyBackAttack')
    front_amp = sv('main.value','AmplifyFrontAttack') + sv('main.extra','AmplifyFrontAttack') + sv('sub','AmplifyFrontAttack') + pct_stone('AmplifyFrontAttack')
    hard_hit = info_effects.get('강타',0.0) + sv('main.value','HardHit') + sv('main.extra','HardHit') + sv('sub','HardHit') + sv('main.value','Hardhit') + sv('main.extra','Hardhit') + sv('sub','Hardhit')
    combat_speed = info_effects.get('전투 속도',0.0) + sv('main.value','CombatSpeed') + sv('main.extra','CombatSpeed') + sv('sub','CombatSpeed')
    ...
    info = {
        ...
        'job': db.get('job') or '',
        'className': db.get('job') or '',
        'combatPower': int(db.get('combatPower') or 0),
        'level': int(db.get('level') or 0),
        'characterLevel': int(db.get('level') or 0),
        'profileImage': db.get('profileImage') or '',
    }
    ...
    for sk in src.get('skills') or []:
        if not isinstance(sk, dict) or not sk.get('acquired'):
            continue
        row = {'code': sk.get('id'), 'name': sk.get('name'), 'level': sk.get('level'), 'category': str(sk.get('category') or '').lower(), 'equip': sk.get('equip')}
        if row['category'] == 'passive':
            if int(row.get('level') or 0) > 0:
                passives.append(row)
        elif row['category'] in {'active','dp'}:
            if int(row.get('level') or 0) > 0:
                active.append(row)
...
async def _official_resolve_character_strict(nickname: str, server: str):
    ...
    # Final identity-only fallback: reuse the already proven normal character search.
    # It supplies only characterId/serverId; all compare stats still come exclusively
    # from the official NC info/equipment/equipment-item APIs below.
    try:
        rows = await search_character_on_server(nickname, server)
        for r in rows or []:
    ...
    return {
        ...
        'skills': [
            {'id': s.get('id'), 'name': s.get('name'), 'category': s.get('category'), 'level': s.get('skillLevel'),
             'acquired': s.get('acquired'), 'equip': s.get('equip')}
