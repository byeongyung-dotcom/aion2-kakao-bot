...
async def openchat_alerts(room: str = ""):
    """Fast, deterministic OpenChat polling endpoint.

    Timed boss/content alerts are calculated locally and never wait on
    external network calls. Each room/event/lead key is handed out once.
    The phone also keeps a second local de-dupe.
    """
    room_key = _openchat_room_key(room)
    _kick_openchat_alert_source_refresh()

    # =====================================================
    # 1) FAST LOCAL PATH: queued test + boss/content alerts
    # =====================================================
    async with _openchat_alert_lock:
        ...
        for row in (delivery.get("testQueue") or []):
            ...
            # Drop stale test requests after 5 minutes.
            if not key or now_epoch - created > 300:
                continue
            ...
            if key not in sent:
                ...
                sent.add(key)
                keep.add(key)
            # test queue is one-shot: do not keep it queued after hand-out.

        delivery["testQueue"] = remaining_tests

        # ---- real scheduled alerts ----
        ...
        for item_type, name, target in targets:
            ...
            # Keep only keys around the current event.
            if minutes > -180:
                for lead in leads:
            ...
            for lead in leads:
                ...
                # 3-minute maximum delivery window, but never extend through
                # the event itself. Examples:
                # 30m lead -> 30.0~27.0m remaining
                # 10m lead -> 10.0~7.0m remaining
                #  2m lead ->  2.0~0.0m remaining
                window = min(3, lead)

                if (
                    max(0, lead - window) < minutes <= lead
                    and key not in sent
                ):
                    items.append({
                        "type": item_type,
                    ...
                    # Atomic one-time hand-out. This prevents old/concurrent
                    # timers from receiving the same alert repeatedly.
                    sent.add(key)
                    keep.add(key)

        delivery["bossSent"] = sorted(sent.intersection(keep))[-300:]
        # Old ACK/inflight claims are intentionally discarded by the new model.
        delivery["inflight"] = {}
    ...
    # =====================================================
    # 2) SLOW PATH: official board alerts only
    # =====================================================
...
async def openchat_alert_ack(room: str = "", key: str = ""):
    # Compatibility endpoint for older phone scripts.
    # Timed alerts are already atomically de-duped at /openchat/alerts.
    return {
        "ok": True,
...
def _queue_openchat_test_alert(room: str):
    ...
    try:
        ...
        queue = list(delivery.get("testQueue") or [])
        queue.append({"key": key, "created": time.time()})
        delivery["testQueue"] = queue[-10:]
        state.setdefault("deliveries", {})[_openchat_delivery_key(room)] = delivery
