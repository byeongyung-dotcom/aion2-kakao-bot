...
async def openchat_alerts(room: str = ""):
    ...
    # Refresh official sources in the background. Timed alerts must never wait
    # for PlayNC/network calls.
    _kick_openchat_alert_source_refresh()

    # =====================================================
    # 1) FAST PATH: boss/content schedule alerts
    # =====================================================
    async with _openchat_alert_lock:
        ...
        # Only one poller may claim a particular alert at a time.
        # If delivery never ACKs, retry after 45 seconds.
        inflight = {}
        now_epoch = time.time()
        for k, claimed_at in (delivery.get("inflight") or {}).items():
            try:
                claimed_at = float(claimed_at)
            except Exception:
                continue
            if now_epoch - claimed_at < 45:
                inflight[str(k)] = claimed_at
        ...
        for item_type, name, target in targets:
            ...
            if minutes > -180:
                for lead in leads:
                    keep.add(event_id + f"|{lead}")

            # Give Android/background polling a 3-minute arrival window.
            # This is NOT a wide catch-up window: e.g. 22:00 / 2m lead means
            # 21:58:00 through 21:59:59 only.
            for lead in leads:
                key = event_id + f"|{lead}"
                if (
                    max(0, lead - 3) < minutes <= lead
                    and key not in sent
                    and key not in inflight
                ):
                    inflight[key] = now_epoch
        ...
        delivery["bossSent"] = sorted(sent.intersection(keep))
        delivery["inflight"] = {
            k: v for k, v in inflight.items()
            if k in keep and k not in sent
        }

        state.setdefault("deliveries", {})[_openchat_delivery_key(room)] = delivery
        _save_openchat_alert_state(state)

        # Critical: if a timed alert is due, return it immediately. Do not make
        # the phone wait for notice/CM/update network requests.
        if schedule_items:
            return {
                "ok": True,
                "enabled": True,
    ...
    # =====================================================
    # 2) SLOW PATH: board alerts
    # Fetch all 3 boards concurrently with a hard timeout.
    # =====================================================
    async def _fetch_alert_board(board_name):
        try:
            return await asyncio.wait_for(
                fetch_board_latest(board_name, limit=18),
                timeout=4.5,
            )
        except Exception:
            return []

    board_names = ("공지", "CM", "업데이트")
    board_results = await asyncio.gather(
        *[_fetch_alert_board(x) for x in board_names]
    )
    latest_by_board = dict(zip(board_names, board_results))
    ...
    async with _openchat_alert_lock:
        ...
        _, delivery = _openchat_get_delivery(state, room)
        board_sent = set(str(x) for x in delivery.get("boardSent", []))
        first_run = not delivery.get("initialized")
        board_items = []
        ...
        if first_run:
            ...
            for board, rows in latest_by_board.items():
                ...
                for post in reversed(new_rows):
                    ...
                    # Board posts remain best-effort de-duped at server receipt;
                    # boss/content delivery uses strict phone ACK.
                    board_sent.add(sent_key)

        delivery["boardSent"] = list(board_sent)[-500:]
        state.setdefault("deliveries", {})[_openchat_delivery_key(room)] = delivery
        _save_openchat_alert_state(state)
...
async def openchat_alert_ack(room: str = "", key: str = ""):
    room_key = _openchat_room_key(room)
    ack_key = str(key or "").strip()
    if not room_key or not ack_key:
        return {"ok": False, "error": "room and key are required"}
    ...
    async with _openchat_alert_lock:
        ...
        sent = set(str(x) for x in delivery.get("bossSent", []))
        sent.add(ack_key)
        delivery["bossSent"] = list(sent)[-200:]
        inflight = dict(delivery.get("inflight") or {})
        inflight.pop(ack_key, None)
        delivery["inflight"] = inflight
        state.setdefault("deliveries", {})[_openchat_delivery_key(room)] = delivery
        saved = _save_openchat_alert_state(state)
...
async def _format_openchat_alert_diagnostic(room: str = ""):
    ...
    for item_type, name, target in targets:
        ...
        for lead in leads:
            key = event_id + f"|{lead}"
            if max(0, lead - 3) < minutes <= lead:
                eligible = lead
                already = key in sent
                claimed = key in inflight
                break
