...
app = FastAPI(title="AION2 Server v57 DataFeedbackCleanUI")
...
async def _full_profile_for_exact_character(nickname: str, server_name: str):
    ...
    for attempt in range(3):
        try:
            ...
            # Accept any payload that actually contains detailed character data.
            # Requiring only info.profile/itemDetails discarded valid stat/skill
            # payloads from alternate NotMeter response shapes.
            has_profile = bool(
...
function renderOptionFeedback(side,x){
  const key=String(side||"").toUpperCase()==="A"?"A":"B";
  const head=$("feedbackHead"+key);
  const box=$("feedback"+key);
  if(!head||!box) return;

  const info=x?.info||{};
  const feedback=x?.optionFeedback||{};
  const rows=Array.isArray(feedback.ranked)&&feedback.ranked.length
    ? feedback.ranked
    : (Array.isArray(feedback.options)?feedback.options:[]);

  head.textContent=`${info.name||key} · ${info.job||"-"}`;

  if(!rows.length){
    box.innerHTML=`<div class="note">옵션 계산에 필요한 공격 스탯 데이터 없음</div>`;
    return;
  }

  box.innerHTML=rows.map((r,i)=>{
    const current=Number(r?.current);
    const gain=Number(r?.gainPct);
    const currentText=Number.isFinite(current)?F(current):"—";
    const gainText=Number.isFinite(gain)?`+${gain.toFixed(3)}%`:"—";
    return `<div class="feedback-row ${i===0?"feedback-best":""}">
      <div>
        <div class="fname">${E(r?.name||r?.stat||"옵션")}</div>
        <div class="fcur">현재 ${E(r?.stat||"")} ${currentText}${r?.stat&&r.stat.includes("증폭")?"%":""} · ${E(r?.mode||"")}</div>
      </div>
      <div class="fcur">+${F(r?.delta||0)}</div>
      <div class="fgain">${gainText}</div>
    </div>`;
  }).join("");
}
...
async function compare(){
  ...
  try{
    ...
    console.error(e);
    $("loadStatus").textContent=`비교 화면 처리 오류: ${e?.message||"알 수 없는 오류"}`;
  }finally{
...
async def debug_v48():
    ...
    return {
        "ok": True,
        "version": "v57-data-feedback-clean-ui",
